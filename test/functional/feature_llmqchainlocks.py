...
class LLMQChainLocksTest(DashTestFramework):
    ...
    def run_test(self):
        ...
        prevcl = cl
        cl0 = self.generate(self.nodes[0], 20, sync_fun=self.no_op)[-6]
        # We need more time here due to 20 blocks being generated at once
        self.wait_for_chainlocked_block_all_nodes(cl0, timeout=30)
        ...
        self.sync_blocks()
        node_height = self.nodes[0].submitchainlock(best_1['blockhash'], best_1['prevblockhash'], best_1['signature'], best_1['signers'])
        prevcl = cl
        ...
        assert best_0['prevblockhash'] != best_1['prevblockhash']
        assert_equal(best_0['known_block'], True)
        # this is a no-op as node1 already has a better CL
        node_height = self.nodes[1].submitchainlock(best_0['blockhash'], best_0['prevblockhash'], best_0['signature'], best_0['signers'])
        # future CLSIG, will not submit
        assert_raises_rpc_error(-8, 'future-clsig', self.nodes[0].submitchainlock, best_1['blockhash'], best_1['prevblockhash'], best_1['signature'], best_1['signers'])
        assert_equal(best_1['height'], node_height)
        assert_equal(best_1['prevblockhash'], prevcl1)
        best_0 = self.nodes[0].getbestchainlock()
        assert best_0['blockhash'] != best_1['blockhash']
        assert best_0['height'] != best_1['height']
        assert best_0['signature'] != best_1['signature']
        assert best_0['prevblockhash'] != best_1['prevblockhash']
        #assert_equal(best_0['prevblockhash'], prevcl)
        assert_equal(best_0['known_block'], True)
        self.reconnect_isolated_node(self.nodes[0], 1)
        self.bump_mocktime(5, nodes=self.nodes)
        self.sync_blocks()
        ...
        cl = self.generate(self.nodes[0], 10, sync_fun=self.no_op)[-6]
        bad_tip = self.nodes[0].getbestblockhash()
        ...
        self.log.info("Try to reorg a CL")

        good_cl = self.nodes[0].getbestblockhash()
        good_tip = self.generate(self.nodes[0], 5, sync_fun=self.no_op)[-1]
        ...
        self.bump_mocktime(5, nodes=self.nodes)
        assert_raises_rpc_error(-1, 'conflicting-with-chainlock', self.generate, self.nodes[0], 5)
        self.wait_for_chainlocked_block_all_nodes(good_cl, timeout=30)
        found = False
        foundConflict = False
        for tip in self.nodes[0].getchaintips():
        ...
        self.log.info("Restart and try again")
        self.stop_node(0)
        self.start_node(0)
        self.nodes[0].invalidateblock(good_cl)
        self.stop_node(0)
        ...
        self.connect_nodes(0, 1)
        force_finish_mnsync(self.nodes[0])
        self.bump_mocktime(10, nodes=self.nodes)
        prev_cl = good_cl
        good_cl = self.nodes[0].getbestblockhash()
        prev_blockcount = self.nodes[0].getblockcount()
        self.generate(self.nodes[0], 5)
        self.wait_for_chainlocked_block_all_nodes(good_cl, timeout=30)
        best_cl = self.nodes[0].getbestchainlock()
        assert_equal(prev_blockcount + 5,  self.nodes[0].getblockcount())
        assert_equal(best_cl["prevblockhash"], prev_cl)
        ...
        self.create_fake_clsig(1, prevcl)
        self.bump_mocktime(5, nodes=self.nodes)
        # no locks formed
        for node in self.nodes:
            self.wait_until(lambda: node.getchainlocks()["recent_chainlock"]["blockhash"] == node0_cl)
            self.wait_until(lambda: node.getchainlocks()["active_chainlock"]["blockhash"] == node0_cl)
    ...
    def create_fake_clsig(self, height_offset, prevblockhash):
        ...
        request_id_buf += bytes.fromhex(prevblockhash)[::-1]
