    ...
    std::optional<int32_t>& floor)
{
    floor.reset();
    if (!state.IsEmpty()) {
        // The first missing audit can depend on older ordinary certificate
        // ancestry. Preserve its roster inputs from marker creation/restart,
        // before the transient dependency walker discovers that ancestry.
        const auto first_target{pq::NextEligibleChainLockTargetHeight(
            finality_config.chainlock_schedule,
            finality_config.activation_predecessor_height)};
        const auto first_roster_floor{first_target
            ? OldestRosterSnapshotHeight(quorum_config, *first_target)
            : std::nullopt};
        if (!first_roster_floor) return false;
        floor = std::min(finality_config.activation_predecessor_height,
                         *first_roster_floor);
    }
...
MakePQChainLockFinalityStoreConfig(const Consensus::Params& consensus)
{
    ...
    config.recovery_refresh = pq::GetRecoveryRefreshConfig(consensus);
...
MakePQQuorumBuildConfig(const Consensus::Params& consensus)
{
    ...
    pq::QuorumBuildConfig config{
        ...
        registry_config.recovery_refresh.IsDisabled()
            ? pq::BTCCScheduleConfig{} : pq::GetBTCCScheduleConfig(consensus),
        pq::GetRecoveryRefreshConfig(consensus)};
    if (!config.IsValid()) return std::nullopt;
    ...
    if (m_config) {
        ...
        m_store = std::make_unique<pq::ChainLockFinalityStore>(
            ...
            [this](
                ...
                const std::optional<pq::BTCCCursorReconciliationProof>&
                    btcc_cursor_reconciliation,
                const pq::ReceiptArchiveRosterAuthorization* covering_authorization,
                const pq::PreparedChainLockContextPtr& context,
                const pq::RecoveryUniverseCapsulePtr& recovery_universe,
                const pq::VerifiedRecoveryResetPersistenceCapability* verified_reset,
                const pq::VerifiedHistoricalSyncSuccessor& historical_successor,
                bool catchup) {
                pq::ChainLockPersistenceError error{pq::ChainLockPersistenceError::NONE};
                std::optional<pq::PaymentAuditSealContextCapsule> seal_context;
                if (!pq::PaymentAuditSealContextCapsule::BuildForVerifiedDurableCandidate(
                        m_genesis_hash, *m_config, chainlock, context, seal_context)) return false;
                const auto publication{BeginChainLockAuxiliarySnapshotPublication()};
                const bool persisted{publication && m_persistence &&
                    m_persistence->PersistBestAfterHistoricalSync(
                        chainlock, context, historical_successor, &error,
                        std::move(seal_context), recovery_universe,
                        btcc_cursor_reconciliation,
                        covering_authorization
                            ? std::optional<pq::ReceiptArchiveRosterAuthorization>{*covering_authorization}
                            : std::nullopt,
                        verified_reset, catchup)};
                // A stale role snapshot rejects C without making the database
                // permanently unavailable. Always close the publication epoch.
                const bool completed{publication &&
                    CompleteChainLockAuxiliarySnapshotPublication(*publication)};
                if (!completed || !m_persistence ||
                    error == pq::ChainLockPersistenceError::IO_FAILURE) {
                    m_persistence_failed.store(true);
                    DisableShareAdmission();
                }
                if (persisted && completed && catchup) m_catchup_used.store(true);
...
{
    {
        ...
        MaintainHistoricalSyncRetention();
        ...
        scheduler->scheduleEvery([this]() EXCLUSIVE_LOCKS_REQUIRED(
            ...
            !m_needed_btcc_certificate_mutex, !m_btcc_preseal_mutex,
            !m_verification_mutex, !m_collector_mutex, !m_lookup_mutex,
            !m_persisted_mutex, !m_signer_reconcile_mutex) {
            while (TryImportPersistedRosterAuthorizationBase() ==
                   PersistedChainLockImport::ACCEPTED) {
            }
            (void)TryImportPersistedChainLock();
            ...
            EnforceBestChainLock();
            MaintainPaymentAuditCheckpointGC();
            const bool historical_dependency{RefreshPoWHistoricalSyncBoundary()};
            MaybeReplayBTCCPreseal();
            const bool payment_dependency{WITH_LOCK(
                m_needed_btcc_certificate_mutex,
                return m_needed_btcc_certificate &&
                    m_needed_btcc_certificate->source ==
                        NeededBTCCCertificateSource::PAYMENT_AUDIT_SEAL)};
            if (!historical_dependency || payment_dependency) {
...
void CChainLocksHandler::Stop()
{
    ...
    std::unique_ptr<CScheduler> scheduler;
    {
        ...
        m_retry_pending_btcc_block.store(false);
        {
            ...
            m_historical_sync_revalidation_input.store({});
            m_historical_sync_servable = {};
            m_historical_sync_bootstrap_servable.reset();
            m_historical_sync_requested.SetNull();
            m_catchup_candidate_hint.reset();
            m_catchup_retry_requested = false;
...
CChainLocksHandler::GetRecoveryUniversePersistenceLookup() const
{
    ...
    return [this](const uint256& source_id) {
        const auto persisted{m_persistence
            ? m_persistence->LoadRecoveryUniverse(source_id)
            : nullptr};
        if (persisted) return persisted;
        const auto input{m_historical_sync_revalidation_input.load()};
        return input && input->recovery_universe &&
                input->recovery_universe->GenesisHash() == m_genesis_hash &&
                input->recovery_universe->SourceId() == source_id
            ? input->recovery_universe : nullptr;
    };
}

void CChainLocksHandler::RetainHistoricalSyncRevalidationInput(
    const HistoricalSyncAuthorization& imported)
{
    AssertLockHeld(cs_main);
    const auto& certificate{imported.base.certificate};
    const auto& boundary{imported.boundary};
    const CBlockIndex* target{certificate
        ? m_chainman.ActiveChain()[certificate->statement.height] : nullptr};
    const CBlockIndex* carrier{
        m_chainman.ActiveChain()[boundary.carrier_height]};
    if (!m_persistence || !certificate || !target || !carrier ||
        !boundary.IsStructurallyValid() ||
        certificate->statement.height != boundary.receipt.chainlock_target_height ||
        certificate->statement.block_hash != boundary.receipt.chainlock_target_hash ||
        target->GetBlockHash() != certificate->statement.block_hash ||
        certificate->GetLogicalId(m_genesis_hash) != boundary.receipt.chainlock_logical_id ||
        carrier->GetBlockHash() != boundary.carrier_hash ||
        carrier->pqBTCCReceiptLogicalId != boundary.receipt.chainlock_logical_id ||
        (target->nStatus & BLOCK_FAILED_MASK) || (carrier->nStatus & BLOCK_FAILED_MASK)) {
        m_historical_sync_revalidation_input.store({});
        UpdateDurableChainLockAuxiliaryRetention();
        return;
    }
    ...
    const auto& source{
        certificate->statement.roster_beacons.active.recovery_authority_source};
    if (!source.IsNull()) {
        // The bootstrap may own the last persisted capsule; retain its bytes
        // before revocation removes that reachability root.
        recovery_universe = GetRecoveryUniversePersistenceLookup()(
            pq::GetRecoveryUniverseSourceId(m_genesis_hash, source));
        const CBlockIndex* source_snapshot{recovery_universe
            ? target->GetAncestor(recovery_universe->SourceSnapshotHeight()) : nullptr};
        if (!source_snapshot ||
            !recovery_universe->Matches(m_genesis_hash, source, *source_snapshot)) {
            m_historical_sync_revalidation_input.store({});
            UpdateDurableChainLockAuxiliaryRetention();
            return;
        }
    ...
    m_historical_sync_revalidation_input.store(
        std::make_shared<const HistoricalSyncRevalidationInput>(
            HistoricalSyncRevalidationInput{certificate, std::move(recovery_universe)}));
    UpdateDurableChainLockAuxiliaryRetention();
}
...
{
    bool pending{true};
    bool resume_participation{false};
    {
        LOCK(cs_main);
...
void CChainLocksHandler::RefreshHistoricalSyncServableRecords()
{
    AssertLockHeld(cs_main);
    m_historical_sync_servable = {};
    m_historical_sync_bootstrap_servable.reset();
    if (!m_persistence) return;
    const auto serving{m_persistence->LoadHistoricalSyncBoundaries()};
    for (std::size_t i{0}; i < serving.size(); ++i) {
        m_historical_sync_servable[i] = std::make_shared<const pq::FinalChainLock>(
            serving[i].record.ChainLock());
    }
    if (const auto bootstrap{m_persistence->LoadHistoricalSyncBootstrap()}) {
        m_historical_sync_bootstrap_servable =
            std::make_shared<const pq::FinalChainLock>(bootstrap->record.ChainLock());
    }
}
...
bool CChainLocksHandler::AlreadyHave(const uint256& logical_id) const
{
    ...
    if (m_store->GetServableByLogicalId(logical_id)) return true;
    LOCK(cs_main);
    if (m_historical_sync_bootstrap_servable &&
        m_historical_sync_bootstrap_servable->GetLogicalId(m_genesis_hash) == logical_id) return true;
    return std::any_of(m_historical_sync_servable.begin(), m_historical_sync_servable.end(),
        [&](const auto& certificate) {
            return certificate && certificate->GetLogicalId(m_genesis_hash) == logical_id;
        });
}
...
{
    if (!m_store || logical_id.IsNull()) return false;
    const auto found{m_store->GetServableByLogicalId(logical_id)};
    if (found) {
        result = *found;
        return true;
    }
    LOCK(cs_main);
    if (m_historical_sync_bootstrap_servable &&
        m_historical_sync_bootstrap_servable->GetLogicalId(m_genesis_hash) == logical_id) {
        result = *m_historical_sync_bootstrap_servable;
        return true;
    }
    for (const auto& historical : m_historical_sync_servable) {
        if (historical && historical->GetLogicalId(m_genesis_hash) == logical_id) {
            result = *historical;
            return true;
        }
    }
    ...
    const CBlockIndex& carrier) const
{
    ...
    if (!chainlock) {
        ...
        if (imported && imported->boundary.receipt == receipt &&
            imported->boundary.carrier_height == carrier.nHeight &&
            imported->boundary.carrier_hash == carrier.GetBlockHash()) {
            chainlock = imported->base.certificate;
        }
    }
...
bool CChainLocksHandler::IsRequiredChainLockCatchupCertificate(
    const uint256& logical_id) const
{
    AssertLockHeld(cs_main);
    if (logical_id.IsNull() || !m_config || !m_store) return false;
    const CBlockIndex* tip{m_chainman.ActiveTip()};
    const auto latest{tip ? pq::LatestEligibleChainLockTargetHeight(
        m_config->chainlock_schedule, tip->nHeight) : std::nullopt};
    const auto best{m_store->GetBestRecord()};
    if (!latest) return false;
    const auto imported{GetPoWHistoricalSyncAuthorization()};
    const auto now{GetTime<std::chrono::microseconds>()};
    if (logical_id == m_historical_sync_requested &&
        (!imported || imported->base.metadata.logical_id != logical_id) &&
        m_historical_sync_last_request.count() && now >= m_historical_sync_last_request &&
        now - m_historical_sync_last_request <= std::chrono::seconds{120} &&
        m_chainman.IsBaseBlockSyncComplete() &&
        (!m_chainman.IsSnapshotActive() || m_chainman.IsSnapshotValidated()) &&
        *latest >= static_cast<int32_t>(m_config->chainlock_schedule.sign_lag)) {
        // B may also have arrived before its history was replayed. Use only
        // indexed lookups here; download permission never mints its proof.
        const CBlockIndex* anchor{tip->GetAncestor(
            *latest - m_config->chainlock_schedule.sign_lag)};
        const auto receipt{anchor ? IndexedBTCCReceiptState(*anchor) : std::nullopt};
        const CBlockIndex* carrier{receipt
            ? anchor->GetAncestor(receipt->latest_receipt_carrier_height) : nullptr};
        if (carrier && carrier->pqBTCCReceiptLogicalId == logical_id &&
            receipt->latest_chainlock_target_height > m_config->activation_predecessor_height &&
            anchor->nHeight >
                (best ? best->metadata.statement.height : m_config->activation_predecessor_height)) {
            return true;
        }
    }
    return m_catchup_retry_requested && m_catchup_candidate_hint && imported &&
        m_catchup_candidate_hint->logical_id == logical_id &&
        m_catchup_candidate_hint->height == *latest &&
        (!best || best->metadata.statement.height < *latest) &&
        tip->GetAncestor(*latest)->GetBlockHash() == m_catchup_candidate_hint->block_hash;
}
    ...
    const uint256& logical_id) const
{
    ...
    if (m_pending_btcc_receipt &&
        ...
        capability.source_token = m_pending_btcc_receipt->carrier_hash;
               ...
               m_needed_btcc_certificate->logical_id == logical_id) {
        switch (m_needed_btcc_certificate->source) {
        case NeededBTCCCertificateSource::LIVE_FRONTIER:
            capability.source = BTCCReceiptArchiveSource::LIVE_FRONTIER;
            break;
        case NeededBTCCCertificateSource::PRESEAL_REPLAY:
            capability.source = BTCCReceiptArchiveSource::PRESEAL_REPLAY;
            break;
        case NeededBTCCCertificateSource::PAYMENT_AUDIT_SEAL:
            return std::nullopt;
        }
...
bool CChainLocksHandler::EraseNeededBTCCCertificateByLogicalId(
    std::optional<NeededBTCCCertificate>& current,
    const uint256& logical_id)
{
    if (!current || logical_id.IsNull() ||
        current->logical_id != logical_id ||
        current->source ==
            NeededBTCCCertificateSource::PAYMENT_AUDIT_SEAL) {
        return false;
...
{
    if (needed && needed->source ==
                      NeededBTCCCertificateSource::PAYMENT_AUDIT_SEAL &&
        !needed->logical_id.IsNull()) {
        return needed->logical_id;
    }
...
{
    LOCK(m_needed_btcc_certificate_mutex);
    (void)EraseNeededBTCCCertificateByLogicalId(
    ...
    const CBlockIndex& carrier)
{
    ...
    // Resolve the active replay dependency first: a later deferred branch
    // must not displace the proof needed to release this node's preseal.
    const PendingPaymentAuditReceiptDependency dependency{
        GetPaymentAuditReplayDependency().value_or(
            PendingPaymentAuditReceiptDependency{
                receipt, carrier.GetBlockHash(),
                carrier.pprev->GetBlockHash()})};
    LOCK(m_pending_payment_audit_receipt_mutex);
    if (m_pending_payment_audit_receipt == dependency) {
        return;
    }
    ...
    m_pending_payment_audit_receipt = dependency;
    m_pending_payment_audit_last_request = std::chrono::microseconds{0};
    LogPrint(BCLog::CHAINLOCKS,
             "CChainLocksHandler::%s -- carrier %s waits for payment "
             "audit %s\n",
             __func__, dependency.carrier_hash.ToString(),
             dependency.receipt.audit_witness_id.ToString());
}
    ...
    const uint256& witness_id) const
{
    ...
    if (dependency->source ==
        PaymentAuditReceiptDependencySource::PRESEAL_REPLAY) {
        if (dependency->source_token.IsNull() ||
            GetPaymentAuditReplayDependency() != dependency) {
            return std::nullopt;
        }
        const CBlockIndex* carrier{
            m_chainman.m_blockman.LookupBlockIndex(
        ...
        if (carrier == nullptr || carrier->pprev == nullptr ||
            carrier->nHeight != dependency->receipt.carrier_height ||
            carrier->pprev->GetBlockHash() !=
                dependency->carrier_parent_hash ||
            m_chainman.ActiveChain()[carrier->nHeight] != carrier ||
            !carrier->IsValid(BLOCK_VALID_SCRIPTS) ||
            !HasFullReceiptIndexProvenance(*carrier)) {
            return std::nullopt;
        }
        ...
        const auto decoded{ExtractDeferredPaymentAuditReceipt(
            carrier_block, witness_id, *carrier, *carrier)};
        if (!decoded || *decoded != dependency->receipt) {
            return std::nullopt;
        }
        // The carrier and source token bind this capability across a null
        // tail advancing the tip; any marker revision invalidates it.
        return PaymentAuditHistoricalContext{
            *dependency, carrier->GetBlockHash(), carrier->nHeight};
    }
    if (dependency->source !=
            PaymentAuditReceiptDependencySource::DEFERRED_CANDIDATE ||
        !dependency->source_token.IsNull()) {
        return std::nullopt;
    }
...
{
    if (expected.dependency.source !=
        PaymentAuditReceiptDependencySource::DEFERRED_CANDIDATE) {
        // An active compact carrier owns a durable replay obligation, not a
        // deferred candidate to retire. Reject the response and retain that
        // obligation; peer-supplied evidence cannot poison local persistence
        // or remove an already connected block through this request path.
        return false;
    }
    ...
    const CBlockIndex& index) const
{
    ...
    std::optional<pq::FinalChainLockRecordMetadata> durable;
    const auto accepted{m_store->GetBestRecordWithDurableSnapshot([&] {
        durable = m_persistence->GetFinalityState().best;
    })};
...
bool CChainLocksHandler::IsPaymentAuditPresealActive() const
{
    ...
    const auto unauthenticated = [&](const auto& marker)
        ...
        return !IsPoWHistoricalPrefixCovered(*terminal) &&
               !IsPaymentAuditPrefixAuthenticated(*terminal);
    };
...
void CChainLocksHandler::UpdateDurableChainLockAuxiliaryRetention()
{
    ...
    const auto inspect_recovery_roster =
        [&](const pq::RecoveryRosterRetentionDependency& dependency) {
            ...
            const auto key_floor{dependency.key_snapshot_height >= 0
                ? std::optional<int32_t>{dependency.key_snapshot_height}
                : pq::RegistrationCutoffHeight(m_quorum_build_config->schedule,
    ...
    if (const auto input{m_historical_sync_revalidation_input.load()}) {
        // A volatile candidate can only lower the durable retention floor;
        // its bytes cannot replace the no-finality activation floor.
        inspect(input->certificate);
    }
    if (!valid) {
...
{
    AssertLockHeld(cs_main);
    // A connected replay boundary must make progress before a later branch
    // can use the same bounded request lane. The getter authenticates the
    // exact active marker, frontier carrier, and current provenance token.
    std::optional<PendingPaymentAuditReceiptDependency> replacement{
        GetPaymentAuditReplayDependency()};
    std::optional<DeferredBTCCReceiptCandidate> best_deferred;
    node::CBlockIndexWorkComparator compare;
    if (!replacement) {
        for (Chainstate* chainstate : m_chainman.GetAll()) {
    ...
    if (replacement) {
        ...
        LogPrint(BCLog::CHAINLOCKS,
                 "CChainLocksHandler::%s -- reconstructing required "
    ...
    if (best_deferred) {
        ...
        if (m_pending_payment_audit_receipt &&
            m_pending_payment_audit_receipt->source ==
                PaymentAuditReceiptDependencySource::DEFERRED_CANDIDATE) {
            for (Chainstate* chainstate : m_chainman.GetAll()) {
    ...
    // Neither an active replay source nor a deferred carrier still owns the
    // old singleton. Never synthesize authority from an orphaned map key.
    if (m_pending_payment_audit_seal) {
        LOCK(m_needed_btcc_certificate_mutex);
        (void)EraseNeededBTCCCertificate(
            m_needed_btcc_certificate,
...
CChainLocksHandler::MakePendingPaymentAuditSealDependency(
    ...
    const std::optional<pq::RosterAuthorizationBaseIdentity>& requested_base,
    uint64_t request_revision)
{
    const bool valid_source{
        owner.source == PaymentAuditReceiptDependencySource::PRESEAL_REPLAY
            ? !owner.source_token.IsNull()
            : owner.source ==
                      PaymentAuditReceiptDependencySource::DEFERRED_CANDIDATE &&
                  owner.source_token.IsNull()};
    if (genesis_hash.IsNull() || !owner.receipt.IsStructurallyValid() ||
        owner.receipt.IsNull() || owner.carrier_hash.IsNull() ||
        owner.carrier_parent_hash.IsNull() || !valid_source ||
        !statement.IsStructurallyValid() ||
        owner.receipt.seal_height != statement.height ||
        owner.receipt.seal_block_hash != statement.block_hash) {
        return std::nullopt;
    }
    const bool initializes{
        ...
        pq::RosterAuthorizationTransitionKind::INITIALIZE};
    if ((initializes &&
         (objective_base || !statement.roster_authorization_base.IsNull())) ||
        (!initializes &&
         (!objective_base || objective_base->IsNull() ||
          *objective_base != statement.roster_authorization_base)) ||
        (requested_base &&
         (!objective_base || requested_base->IsNull() ||
          !requested_base->IsStructurallyValid() ||
          requested_base->height > objective_base->height ||
          requested_base->height >= statement.height ||
          (requested_base->height == objective_base->height &&
           *requested_base != *objective_base)))) {
        return std::nullopt;
    }
    const uint256 logical_id{
        pq::GetLogicalChainLockId(genesis_hash, statement)};
    if (logical_id.IsNull()) return std::nullopt;

    CHashWriter writer{SER_GETHASH, 0};
    writer << std::string{"SYS_PQ_PAYMENT_AUDIT_SEAL_FETCH_V2"}
           << owner.receipt << owner.carrier_hash
           << owner.carrier_parent_hash
           << static_cast<uint8_t>(owner.source) << owner.source_token
           << statement
           << (objective_base ? *objective_base
                              : pq::RosterAuthorizationBaseIdentity{})
           << logical_id << requested_base.has_value()
           << requested_base.value_or(pq::RosterAuthorizationBaseIdentity{})
           << request_revision;
    const uint256 source_token{writer.GetHash()};
    if (source_token.IsNull()) return std::nullopt;
    return PendingPaymentAuditSealDependency{
        owner, statement,
        objective_base.value_or(
            pq::RosterAuthorizationBaseIdentity{}),
        logical_id, source_token, requested_base, request_revision};
}

bool CChainLocksHandler::PublishPendingPaymentAuditSealDependency(
    std::optional<PendingPaymentAuditSealDependency>& current,
    const PendingPaymentAuditSealDependency& dependency)
{
    if (dependency.logical_id.IsNull() || dependency.source_token.IsNull()) {
        return false;
    }
...
bool CChainLocksHandler::DoesPaymentAuditSealSourceMatch(
    ...
    const std::optional<NeededBTCCCertificate>& needed) noexcept
{
    const auto& dependency{capability.dependency};
    // This is download ownership, not ordinary verification authority. A
    // parent lost to retention must leave the request consumable so the
    // full verifier below can descend and recover that parent again.
    const bool exact_base{
        !capability.objective_base ||
            (!dependency.objective_base.IsNull() &&
                capability.objective_base->certificate &&
                capability.objective_base->verification_context &&
                capability.objective_base->metadata.AuthorizationBase() ==
                    dependency.objective_base &&
                capability.objective_base->metadata.IsInternallyConsistent(
                    capability.objective_base->verification_context
                        ->GenesisHash()) &&
                capability.objective_base->certificate->statement ==
                    capability.objective_base->metadata.statement &&
                capability.objective_base->verification_context
                        ->Statement() ==
                    capability.objective_base->metadata.statement)};
    return exact_base && pending && *pending == dependency.owner && seal &&
        *seal == dependency && needed &&
        needed->source ==
            NeededBTCCCertificateSource::PAYMENT_AUDIT_SEAL &&
        needed->logical_id == dependency.RequestedIdentity().logical_id &&
        needed->source_token == dependency.source_token;
}

bool CChainLocksHandler::StagePendingPaymentAuditSealDependency(
    const PendingPaymentAuditSealDependency& dependency)
{
    if (!m_store) return false;
    ...
    auto staged{dependency};
    if (!already_servable && !objective_base &&
        !dependency.objective_base.IsNull() && !dependency.requested_base) {
        const auto requested{MakePendingPaymentAuditSealDependency(
            m_genesis_hash, dependency.owner, dependency.statement,
            dependency.objective_base, dependency.objective_base,
            dependency.request_revision)};
        if (!requested) return false;
        staged = *requested;
    }
    LOCK(m_pending_payment_audit_receipt_mutex);
    if (!m_pending_payment_audit_receipt ||
        *m_pending_payment_audit_receipt != dependency.owner) {
        return false;
    }
    LOCK(m_needed_btcc_certificate_mutex);
    if (already_servable) {
    ...
    if (m_pending_payment_audit_seal &&
        m_pending_payment_audit_seal->owner == dependency.owner &&
        m_pending_payment_audit_seal->statement == dependency.statement &&
        m_pending_payment_audit_seal->objective_base ==
            dependency.objective_base) {
        // A duplicate audit cannot restart an in-flight descent or revive
        // the source token from an earlier request of the same identity.
        return true;
    }
    if (m_pending_payment_audit_seal &&
        m_pending_payment_audit_seal->source_token !=
    ...
    (void)PublishPendingPaymentAuditSealDependency(
        m_pending_payment_audit_seal, staged);
    (void)PublishNeededBTCCCertificate(
        m_needed_btcc_certificate,
        NeededBTCCCertificateSource::PAYMENT_AUDIT_SEAL,
        staged.RequestedIdentity().logical_id, staged.source_token);
    return DoesPaymentAuditSealSourceMatch(
        PaymentAuditSealFetchCapability{
            staged, std::move(objective_base)},
        m_pending_payment_audit_receipt,
        m_pending_payment_audit_seal,
        m_needed_btcc_certificate);
}

bool CChainLocksHandler::AdvancePaymentAuditSealDependency(
    const PaymentAuditSealFetchCapability& capability,
    const pq::RosterAuthorizationBaseIdentity& missing_base)
{
    const auto& previous{capability.dependency};
    if (missing_base.IsNull() || !missing_base.IsStructurallyValid() ||
        missing_base.height >= previous.RequestedIdentity().height ||
        previous.request_revision == std::numeric_limits<uint64_t>::max()) {
        return false;
    }
    const auto next{MakePendingPaymentAuditSealDependency(
        m_genesis_hash, previous.owner, previous.statement,
        previous.objective_base, missing_base,
        previous.request_revision + 1)};
    if (!next) return false;

    LOCK(cs_main);
    const auto owner{ResolvePendingPaymentAuditContext(
        previous.owner.receipt.audit_witness_id)};
    if (!owner || owner->dependency != previous.owner) return false;
    const CBlockIndex* carrier{m_chainman.m_blockman.LookupBlockIndex(
        previous.owner.carrier_hash)};
    const auto requested{previous.RequestedIdentity()};
    const CBlockIndex* current{carrier
        ? carrier->GetAncestor(requested.height) : nullptr};
    const CBlockIndex* target{carrier
        ? carrier->GetAncestor(missing_base.height) : nullptr};
    if (!current || current->GetBlockHash() != requested.block_hash ||
        !target || target->GetBlockHash() != missing_base.block_hash) {
        return false;
    }
    std::optional<pq::RosterAuthorizationBaseIdentity> selected;
    (void)ResolveObjectiveRosterAuthorizationContext(
        *current, nullptr, &selected);
    if (!selected || *selected != missing_base) return false;
    LOCK(m_pending_payment_audit_receipt_mutex);
    LOCK(m_needed_btcc_certificate_mutex);
    if (!DoesPaymentAuditSealSourceMatch(
            capability, m_pending_payment_audit_receipt,
            m_pending_payment_audit_seal, m_needed_btcc_certificate)) {
        return false;
    }
    m_pending_payment_audit_seal = *next;
    (void)PublishNeededBTCCCertificate(
        m_needed_btcc_certificate,
        NeededBTCCCertificateSource::PAYMENT_AUDIT_SEAL,
        next->RequestedIdentity().logical_id, next->source_token);
    return true;
}
...
CChainLocksHandler::GetPaymentAuditSealFetchCapability(
    const uint256& logical_id) const
{
    if (!m_store || logical_id.IsNull()) return std::nullopt;
    std::optional<PendingPaymentAuditSealDependency> dependency;
    {
        LOCK(m_pending_payment_audit_receipt_mutex);
        LOCK(m_needed_btcc_certificate_mutex);
        if (!m_pending_payment_audit_seal ||
            m_pending_payment_audit_seal->RequestedIdentity().logical_id !=
                logical_id) {
            return std::nullopt;
        }
        dependency = m_pending_payment_audit_seal;
    }
    std::optional<pq::VerifiedRosterAuthorizationBaseView> objective_base;
    ...
    PaymentAuditSealFetchCapability capability{
        *dependency, std::move(objective_base)};
    LOCK(m_pending_payment_audit_receipt_mutex);
    LOCK(m_needed_btcc_certificate_mutex);
    if (!DoesPaymentAuditSealSourceMatch(
            capability, m_pending_payment_audit_receipt,
...
bool CChainLocksHandler::IsPaymentAuditSealFetchCapabilityCurrent(
    const PaymentAuditSealFetchCapability& capability) const
{
    const auto current{GetPaymentAuditSealFetchCapability(
        capability.dependency.RequestedIdentity().logical_id)};
    if (!current || current->dependency != capability.dependency ||
        current->objective_base.has_value() !=
            capability.objective_base.has_value()) {
        return false;
    }
    return !current->objective_base ||
        current->objective_base->metadata ==
            capability.objective_base->metadata;
}

bool CChainLocksHandler::AuthorizePaymentAuditSealPersistence(
    const PaymentAuditSealFetchCapability& capability,
    const std::function<bool()>& persist_record,
    pq::ChainLockFinalityError* error) const
{
    if (!persist_record || !m_store) {
        if (error != nullptr) {
            *error = pq::ChainLockFinalityError::CONTEXT_CHANGED;
        }
        return false;
    }
    // Marker recovery can precede its engine replay's chainstate lock. Keep
    // the actual owner revision stable through the certificate fsync, rather
    // than relying only on the still-published request tuple.
    LOCK(cs_main);
    const auto owner{ResolvePendingPaymentAuditContext(
        capability.dependency.owner.receipt.audit_witness_id)};
    if (!owner || owner->dependency != capability.dependency.owner) {
        if (error != nullptr) {
            *error = pq::ChainLockFinalityError::CONTEXT_CHANGED;
        }
        return false;
    ...
    LOCK(m_pending_payment_audit_receipt_mutex);
    LOCK(m_needed_btcc_certificate_mutex);
    const bool source_current{DoesPaymentAuditSealSourceMatch(
        capability, m_pending_payment_audit_receipt,
        m_pending_payment_audit_seal,
        m_needed_btcc_certificate)};
    if (!source_current) {
...
void CChainLocksHandler::CompletePaymentAuditSealFetch(
    const PaymentAuditSealFetchCapability& capability)
{
    std::optional<PendingPaymentAuditSealDependency> next;
    if (capability.dependency.requested_base) {
        const auto& previous{capability.dependency};
        if (!m_store || previous.request_revision ==
                            std::numeric_limits<uint64_t>::max()) {
            return;
        }
        const auto base{m_store->GetVerifiedRosterAuthorizationBase(
            previous.objective_base)};
        next = MakePendingPaymentAuditSealDependency(
            m_genesis_hash, previous.owner, previous.statement,
            previous.objective_base,
            base ? std::optional<pq::RosterAuthorizationBaseIdentity>{}
    ...
    (void)EraseNeededBTCCCertificate(
        m_needed_btcc_certificate,
        NeededBTCCCertificateSource::PAYMENT_AUDIT_SEAL,
        capability.dependency.source_token);
    m_pending_payment_audit_seal = next;
    if (next) {
        (void)PublishNeededBTCCCertificate(
            m_needed_btcc_certificate,
...
CChainLocksHandler::BuildPaymentAuditSealVerificationContext(
    const PaymentAuditSealFetchCapability& capability,
    const pq::ChainLockStatement& statement,
    bool publish_roster,
    std::optional<pq::RosterAuthorizationBaseIdentity>* missing_base) const
{
    if (missing_base != nullptr) missing_base->reset();
    if (!m_config || !m_quorum_build_config || !m_store) {
        return std::nullopt;
    }
    const auto& dependency{capability.dependency};
    const auto rebuilt{MakePendingPaymentAuditSealDependency(
        m_genesis_hash, dependency.owner, dependency.statement,
        dependency.objective_base.IsNull()
            ? std::optional<pq::RosterAuthorizationBaseIdentity>{}
            : std::optional<pq::RosterAuthorizationBaseIdentity>{
                  dependency.objective_base},
        dependency.requested_base, dependency.request_revision)};
    if (!rebuilt || *rebuilt != dependency) return std::nullopt;
    const pq::RosterAuthorizationBaseIdentity received{
        statement.height, statement.block_hash,
        pq::GetLogicalChainLockId(m_genesis_hash, statement)};
    if (!statement.IsStructurallyValid() ||
        received != dependency.RequestedIdentity() ||
        (!dependency.requested_base && statement != dependency.statement) ||
        (dependency.requested_base &&
         !pq::IsBTCCCandidateHeight(m_config->btcc_schedule,
                                   statement.height))) {
        return std::nullopt;
    }
    ...
    const CBlockIndex* seal{nullptr};
    std::optional<ObjectiveRosterAuthorizationContext> objective;
    std::optional<pq::RosterAuthorizationVerificationContext>
        authorization;
    {
        LOCK(cs_main);
        const auto historical{ResolvePendingPaymentAuditContext(
            dependency.owner.receipt.audit_witness_id)};
        if (!historical || historical->dependency != dependency.owner) {
            return std::nullopt;
        }
        const pq::PaymentAuditScheduleConfig audit_config{
            m_config->chainlock_schedule, m_config->btcc_schedule};
        const auto audit_schedule{pq::BuildPaymentAuditEpochSchedule(
            audit_config, dependency.owner.receipt.epoch)};
        seal = m_chainman.m_blockman.LookupBlockIndex(
            statement.block_hash);
        const CBlockIndex* carrier{
            m_chainman.m_blockman.LookupBlockIndex(
                dependency.owner.carrier_hash)};
        if (!audit_schedule ||
            audit_schedule->seal_height != dependency.statement.height ||
            !pq::PaymentAuditCarrierWindow{
                 dependency.owner.receipt.epoch,
                 audit_schedule->carrier_start_height,
                 audit_schedule->carrier_end_height_exclusive}
                 .Contains(dependency.owner.receipt.carrier_height) ||
            carrier == nullptr || carrier->pprev == nullptr ||
            carrier->nHeight != dependency.owner.receipt.carrier_height ||
            carrier->pprev->GetBlockHash() !=
                dependency.owner.carrier_parent_hash ||
            statement.previous_chainlock_height <
                m_config->activation_predecessor_height ||
            carrier->GetAncestor(statement.height) != seal ||
        ...
        if (statement.roster_transition !=
            pq::RosterAuthorizationTransitionKind::INITIALIZE) {
            const auto ordinary_base{
                m_store->GetVerifiedRosterAuthorizationBase(
                    statement.roster_authorization_base)};
            std::optional<pq::RosterAuthorizationBaseIdentity> selected;
            objective = ResolveObjectiveRosterAuthorizationContext(
                *seal, ordinary_base ? &*ordinary_base : nullptr, &selected);
            if (!selected || *selected != statement.roster_authorization_base ||
                selected->height >= statement.height) {
                return std::nullopt;
            }
            if (!ordinary_base) {
                if (missing_base != nullptr) *missing_base = *selected;
                return std::nullopt;
            }
            if (!objective || !objective->base ||
                objective->base->metadata.AuthorizationBase() !=
                    statement.roster_authorization_base) {
                return std::nullopt;
            }
        } else if (!statement.roster_authorization_base.IsNull()) {
            return std::nullopt;
        }
    }
    // The ordinary authorization builder samples chain state internally and
    // may perform roster work; it must run without cs_main held.
    authorization = BuildNetworkRosterAuthorizationContext(
        statement, *seal, objective ? &*objective : nullptr);
    if (!authorization || seal == nullptr) return std::nullopt;

    pq::QuorumBuildError build_error{pq::QuorumBuildError::NONE};
    const auto roster_set{publish_roster
        ? roster_cache->GetVerifiedActive(
              statement.height, *seal,
    ...
    auto prepared{pq::PreparedChainLockContext::Create(
        ...
        roster_set, *authorization, &verification_error)};
    if (!prepared) return std::nullopt;
    return RuntimeVerificationContext{
        std::move(prepared), HistoricalAdmissionContext{},
        roster_source_generation};
}
    ...
    const uint256& logical_id) const
{
    ...
    const auto inspect_payment_audit_marker = [&](const auto& marker,
        ...
        if (winner_covers || IsPoWHistoricalPrefixCovered(*terminal)) return;
    ...
    if (!best && target_is_active &&
        ...
        if (covered != nullptr &&
            ...
            AssertLockNotHeld(m_lookup_mutex);
...
const CBlockIndex* CChainLocksHandler::ResolvePoWHistoricalSelectionTip(
    const CBlockIndex& active_tip,
    const pq::ChainLockScheduleConfig& schedule,
    std::optional<int32_t> coverage_height)
{
    if (!schedule.IsValid()) return nullptr;
    if (!coverage_height) return &active_tip;
    const int64_t selection_height{
        int64_t{*coverage_height} + 2 * int64_t{schedule.sign_lag}};
    if (*coverage_height < 0 || selection_height > active_tip.nHeight) {
        return nullptr;
    }
    const auto target{pq::LatestEligibleChainLockTargetHeight(
        schedule, static_cast<int32_t>(selection_height))};
    if (!target || int64_t{*target} - schedule.sign_lag != *coverage_height) {
        return nullptr;
    }
    return active_tip.GetAncestor(static_cast<int32_t>(selection_height));
}

void CChainLocksHandler::RevokeReorgedHistoricalSyncAuthorization()
{
    LOCK(cs_main);
    RevokeReorgedHistoricalSyncAuthorizationLocked();
}

bool CChainLocksHandler::TryReenterHistoricalSyncAuthentication(
    const HistoricalSyncAuthorization& imported)
{
    AssertLockHeld(cs_main);
    if (!m_config || !m_store || !m_persistence || m_persistence_failed.load() ||
        !m_payment_audit_store || !m_payment_audit_staging_store ||
        !m_payment_audit_store->IsHealthy() || !m_payment_audit_staging_store->IsHealthy() ||
        (m_chainman.IsSnapshotActive() && !m_chainman.IsSnapshotValidated())) return false;
    {
        LOCK(m_persisted_mutex);
        if (m_pending_persisted || m_pending_persisted_unsealed_btcc ||
            !m_pending_persisted_authorization_bases.empty() || m_persisted_invalid ||
            m_persisted_best_auth_pending || m_persisted_unsealed_auth_pending) return false;
    }
    const auto& boundary{imported.boundary};
    const auto& base{imported.base};
    if (!boundary.IsStructurallyValid() || !base.certificate || !base.verification_context ||
        base.certificate->statement != base.metadata.statement ||
        base.verification_context->Statement() != base.metadata.statement ||
        base.verification_context->StatementLogicalId() != base.metadata.logical_id ||
        base.metadata.logical_id != boundary.receipt.chainlock_logical_id ||
        base.certificate->GetLogicalId(m_genesis_hash) != base.metadata.logical_id ||
        base.metadata.statement.height != boundary.receipt.chainlock_target_height ||
        base.metadata.statement.block_hash != boundary.receipt.chainlock_target_hash) return false;
    std::optional<pq::FinalChainLockRecordMetadata> durable;
    const auto accepted{m_store->GetBestRecordWithDurableSnapshot([&] {
        durable = m_persistence->GetFinalityState().best;
    })};
    if (!IsHistoricalSyncRetentionMutationReady(
            accepted ? &accepted->metadata : nullptr,
            durable ? &*durable : nullptr, m_chainman.ActiveChain())) return false;
    const CBlockIndex* old_end{m_chainman.m_blockman.LookupBlockIndex(boundary.coverage_hash)};
    const CBlockIndex* tip{m_chainman.ActiveTip()};
    const CBlockIndex* previous_floor{boundary.durable_prior.IsNull() ? nullptr
        : m_chainman.m_blockman.LookupBlockIndex(boundary.durable_prior.block_hash)};
    const CBlockIndex* current_floor{durable
        ? m_chainman.ActiveChain()[durable->statement.height] : nullptr};
    if (!old_end || old_end->nHeight != boundary.coverage_height || !tip ||
        (!boundary.durable_prior.IsNull() &&
         (!previous_floor || previous_floor->nHeight != boundary.durable_prior.height)) ||
        (durable && (!current_floor || current_floor->GetBlockHash() != durable->statement.block_hash)) ||
        (durable && durable->statement.height == boundary.durable_prior.height &&
         durable->AuthorizationBase() != boundary.durable_prior)) return false;

    const auto pending_archive{m_payment_audit_store->GetPendingPruneCheckpoint()};
    const auto pending_probation{deterministicMNManager
        ? deterministicMNManager->GetPendingPaymentProbationGCRequest() : std::nullopt};
    const auto completed_archive{m_payment_audit_store->GetPruneCheckpoint()};
    const std::optional<pq::PaymentAuditStoreCheckpoint> checkpoint{pending_archive
        ? pending_archive : pending_probation
            ? std::optional<pq::PaymentAuditStoreCheckpoint>{pending_probation->checkpoint}
            : completed_archive};
    if (checkpoint) {
        const CBlockIndex* authorizer{m_chainman.m_blockman.LookupBlockIndex(
            checkpoint->authorizing_target_hash)};
        if (!checkpoint->IsStructurallyValid() || !authorizer ||
            authorizer->nHeight != checkpoint->authorizing_target_height ||
            !IsPaymentAuditCheckpointAuthenticated(*checkpoint, *authorizer)) return false;
    }
    ...
    const auto previously_covered = [&](const auto& marker)
        EXCLUSIVE_LOCKS_REQUIRED(cs_main) -> const CBlockIndex* {
        if (!marker || !marker->IsStructurallyValid() ||
            marker->terminal_carrier_height <= m_config->activation_predecessor_height ||
            marker->terminal_carrier_height > old_end->nHeight) return nullptr;
        const CBlockIndex* terminal{old_end->GetAncestor(marker->terminal_carrier_height)};
        return terminal && terminal->GetBlockHash() == marker->terminal_carrier_hash
            ? terminal : nullptr;
    };
    const auto active_terminal = [&](const auto& marker)
        EXCLUSIVE_LOCKS_REQUIRED(cs_main) -> const CBlockIndex* {
        if (!marker || !marker->IsStructurallyValid()) return nullptr;
        const CBlockIndex* terminal{m_chainman.ActiveChain()[marker->terminal_carrier_height]};
        return terminal && terminal->GetBlockHash() == marker->terminal_carrier_hash
            ? terminal : nullptr;
    };
    bool revoked_dependency{false};
    const auto recognize_unresolved = [&](const auto& marker)
        EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
        if (!previously_covered(marker)) return false;
        revoked_dependency = true;
        return true;
    };
    const auto inspect_btcc = [&](const auto& marker) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
        if (!marker) return true;
        const CBlockIndex* terminal{active_terminal(marker)};
        if (terminal && (IsBTCCPrefixAuthenticated(*terminal) ||
            CheckBTCCReceiptCertificate(marker->terminal_receipt, *terminal) ==
                BTCCReceiptCertificateStatus::VERIFIED)) return true;
        if (!terminal && marker->IsStructurallyValid()) {
            const CBlockIndex* old_carrier{m_chainman.m_blockman.LookupBlockIndex(
                marker->terminal_carrier_hash)};
            // A receipt just beyond E can have been authenticated by D=C-10
            // without finalizing C. Its old-branch witness remains independent
            // evidence when both C and E disconnect, but cannot trigger reentry.
            if (old_carrier && old_carrier->nHeight == marker->terminal_carrier_height &&
                old_carrier->nHeight > old_end->nHeight &&
                old_carrier->GetAncestor(old_end->nHeight) == old_end &&
                old_carrier->pqBTCCReceiptLogicalId == marker->terminal_receipt.chainlock_logical_id &&
                CheckBTCCReceiptCertificate(marker->terminal_receipt, *old_carrier) ==
                    BTCCReceiptCertificateStatus::VERIFIED) return true;
        }
        return recognize_unresolved(marker);
    };
    const auto inspect_payment = [&](const auto& marker) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
        if (!marker) return true;
        const CBlockIndex* terminal{active_terminal(marker)};
        if (terminal && IsPaymentAuditPrefixAuthenticated(*terminal)) return true;
        return recognize_unresolved(marker);
    };
    // Explain every unresolved marker, not just one coincident revoked dependency.
    if (!inspect_btcc(btcc.active) || !inspect_btcc(btcc.prospective) ||
        !inspect_payment(payment.active) || !inspect_payment(payment.prospective) ||
        !revoked_dependency) return false;
    return m_chainman.TryReenterPendingPQHistoryAuthentication(PQHistoryReauthentication{
        m_chainman, *old_end, *tip, previous_floor, current_floor,
        PresealAdmissionToken(btcc, payment), imported.provenance_revision});
}

void CChainLocksHandler::RevokeReorgedHistoricalSyncAuthorizationLocked()
{
    AssertLockHeld(cs_main);
    if (m_historical_sync && !GetPoWHistoricalSyncAuthorization()) {
        RetainHistoricalSyncRevalidationInput(*m_historical_sync);
        if (TryReenterHistoricalSyncAuthentication(*m_historical_sync)) {
            m_historical_sync_reauthentication_pending.store(true);
            // Close without the terminal-failure latch; old signing tokens stay revoked.
            while (!m_share_admission_gate.TryPublishEnabled(
                m_share_admission_gate.Observe(), false)) {}
            (void)m_auxiliary_history_gc_auth_gate.SetHealthy(false, [this] {
                return RevokeAuxiliaryHistoryGCAuthorization();
...
bool CChainLocksHandler::IsHistoricalSyncRetentionMutationReady(
    const pq::FinalChainLockRecordMetadata* accepted,
    const pq::FinalChainLockRecordMetadata* durable, const CChain& active_chain)
{
    AssertLockHeld(cs_main);
    if ((accepted == nullptr) != (durable == nullptr) ||
        (accepted && *accepted != *durable)) return false;
    if (!durable) return true;
    const CBlockIndex* winner{active_chain[durable->statement.height]};
    return winner && winner->GetBlockHash() == durable->statement.block_hash &&
...
void CChainLocksHandler::MaintainHistoricalSyncRetention()
{
    ...
    if (!admission_lock) return;
    (void)m_chainman.ActiveChainstate().RunWithStableActiveChain([&] {
        LOCK(cs_main);
        RevokeReorgedHistoricalSyncAuthorizationLocked();
        uint64_t revision{0};
        auto retained{m_persistence->LoadHistoricalSyncBoundaries(&revision)};
        const bool replayed{m_chainman.IsBaseBlockSyncComplete() &&
            (!m_chainman.IsSnapshotActive() || m_chainman.IsSnapshotValidated())};
        const CBlockIndex* tip{m_chainman.ActiveTip()};
        const auto cache_records = [&]() EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
            RefreshHistoricalSyncServableRecords();
        };
        cache_records();
        if (!replayed || !tip) return true;
        const auto accepted{m_store->GetBestRecord()};
        const auto durable{m_persistence->GetFinalityState().best};
        // Pending startup finality may not yet be enforced on this active
        // branch. Preserve its exact B dependency until restoration converges.
        if (!IsHistoricalSyncRetentionMutationReady(
                accepted ? &accepted->metadata : nullptr,
                durable ? &*durable : nullptr, m_chainman.ActiveChain())) return true;
        // Restart records are bytes, not authority. A changed supporting
        // branch removes the serving slot until ordinary revalidation.
        for (std::size_t i{0}; i < retained.size(); ++i) {
            const auto& stored{retained[i]};
            const auto& boundary{stored.boundary};
            ...
            const bool carrier_matches{carrier &&
                carrier->GetBlockHash() == boundary.carrier_hash &&
                carrier->pqBTCCReceiptLogicalId == boundary.receipt.chainlock_logical_id};
            const bool end_matches{end &&
                end->GetBlockHash() == boundary.coverage_hash};
            if (carrier_matches && end_matches) continue;
            m_historical_sync_servable[i].reset();
            pq::ChainLockPersistenceError error{pq::ChainLockPersistenceError::NONE};
            if (!m_persistence->InvalidateHistoricalSyncBoundary(
                    stored.record.RecordIdentity(), revision, &error)) {
        ...
        if (const auto bootstrap{m_persistence->LoadHistoricalSyncBootstrap()}) {
            ...
            if (!carrier || !end || carrier->GetBlockHash() != boundary.carrier_hash ||
                ...
                if (m_historical_sync && m_historical_sync->record_identity ==
                        bootstrap->record.RecordIdentity()) m_historical_sync.reset();
                m_historical_sync_bootstrap_servable.reset();
                pq::ChainLockPersistenceError error{pq::ChainLockPersistenceError::NONE};
                if (!m_persistence->InvalidateHistoricalSyncBootstrap(
                        bootstrap->record.RecordIdentity(), revision, &error)) {
                    if (error == pq::ChainLockPersistenceError::IO_FAILURE) {
                        m_persistence_failed.store(true);
                        DisableShareAdmission();
                    }
        ...
        const CBlockIndex* best{tip->GetAncestor(durable->statement.height)};
        const auto height{pq::LatestEligibleChainLockTargetHeight(
            m_config->chainlock_schedule, tip->nHeight)};
        const CBlockIndex* lookup{height && *height >=
                static_cast<int32_t>(m_config->chainlock_schedule.sign_lag)
            ? tip->GetAncestor(*height - m_config->chainlock_schedule.sign_lag)
            : nullptr};
        if (!best || best->GetBlockHash() != durable->statement.block_hash ||
            !lookup || !HasFullReceiptIndexProvenance(*lookup)) return true;
        const auto receipt_state{IndexedBTCCReceiptState(*lookup)};
        const CBlockIndex* carrier{receipt_state
            ? lookup->GetAncestor(receipt_state->latest_receipt_carrier_height)
            : nullptr};
        if (!carrier || !carrier->pprev || carrier->pqBTCCReceiptLogicalId.IsNull()) return true;
        if (!retained.empty() && retained.front().boundary.carrier_height == carrier->nHeight &&
            retained.front().boundary.carrier_hash == carrier->GetBlockHash()) return true;
        const auto before{IndexedBTCCReceiptState(*carrier->pprev)};
        const auto after{IndexedBTCCReceiptState(*carrier)};
        const auto audit{IndexedPaymentAuditReceiptState(*carrier)};
        if (!before || !after || !audit || *after != *receipt_state ||
            !HasFullReceiptIndexProvenance(*carrier)) return true;
        const auto receipt{pq::ReconstructBTCCReceipt(
            m_genesis_hash, m_config->chainlock_schedule, m_config->btcc_schedule,
            m_config->activation_predecessor_height, *carrier, *before, *after,
            carrier->pqBTCCReceiptLogicalId)};
        const auto base{receipt ? m_store->GetVerifiedRosterAuthorizationBaseByLogicalId(
            receipt->chainlock_logical_id) : std::nullopt};
        if (!receipt || !base || !base->certificate || !base->verification_context ||
            base->verification_context->Authorization().admission ==
                pq::RosterAuthorizationAdmission::POW_HISTORY ||
            base->metadata.statement.height > best->nHeight ||
            best->GetAncestor(base->metadata.statement.height)->GetBlockHash() !=
                base->metadata.statement.block_hash) return true;
        // This retains an already durable, fully verified certificate. It
        // never constructs a PoW authorization capability or changes best.
        pq::HistoricalSyncBoundary boundary;
        boundary.durable_prior = durable->AuthorizationBase();
        boundary.carrier_height = carrier->nHeight;
        boundary.carrier_hash = carrier->GetBlockHash();
        boundary.receipt = *receipt;
        boundary.coverage_height = carrier->nHeight;
        boundary.coverage_hash = carrier->GetBlockHash();
        boundary.receipt_state = *after;
        boundary.payment_audit_state = *audit;
        boundary.probation_state_hash = carrier->pqPaymentProbationStateHash;
        pq::RecoveryUniverseCapsulePtr universe;
        if (!boundary.IsStructurallyValid() ||
            !CaptureRecoveryUniverseForDurableCandidate(*base->certificate, universe)) return true;
        const auto publication{BeginChainLockAuxiliarySnapshotPublication()};
        if (!publication) return false;
        std::optional<pq::FinalChainLockRecordMetadata> covering;
        if (!retained.empty()) {
            const auto& current{retained.front().boundary};
            const CBlockIndex* covered{best->GetAncestor(current.carrier_height)};
            if (covered && covered->GetBlockHash() == current.carrier_hash) covering = durable;
        }
        pq::ChainLockPersistenceError error{pq::ChainLockPersistenceError::NONE};
        const bool flushed{m_chainman.m_blockman.FlushChainstateBlockFile(carrier->nHeight) &&
            FlushBTCCIndexStateForDurableAcceptanceLocked(*base->certificate)};
        const bool saved{flushed && m_persistence->PersistHistoricalSyncBoundary(
            *base->certificate, base->verification_context, boundary, revision,
            covering, universe, &error)};
        if (!CompleteChainLockAuxiliarySnapshotPublication(*publication)) return false;
        if (!flushed || error == pq::ChainLockPersistenceError::IO_FAILURE) {
            m_persistence_failed.store(true);
            DisableShareAdmission();
            return false;
        }
        if (saved) {
            retained = m_persistence->LoadHistoricalSyncBoundaries();
            cache_records();
        }
        return true;
    });
...
std::optional<pq::HistoricalSyncBoundary>
CChainLocksHandler::SelectPoWHistoricalSyncBoundary(
    std::optional<int32_t> coverage_height) const
{
    AssertLockHeld(cs_main);
    if (!m_config || !m_store || !m_persistence ||
        m_persistence_failed.load() ||
    ...
    const CBlockIndex* active_tip{m_chainman.ActiveTip()};
    const CBlockIndex* tip{active_tip
        ? ResolvePoWHistoricalSelectionTip(
              *active_tip, m_config->chainlock_schedule, coverage_height)
        : nullptr};
    const auto height{tip ? pq::LatestEligibleChainLockTargetHeight(
        m_config->chainlock_schedule, tip->nHeight) : std::nullopt};
    if (!height || *height <
            static_cast<int32_t>(m_config->chainlock_schedule.sign_lag)) {
        return std::nullopt;
    }
    const CBlockIndex* authority{tip->GetAncestor(
        *height - m_config->chainlock_schedule.sign_lag)};
    const CBlockIndex* coverage{authority};
    if (!coverage || !authority) return std::nullopt;
    const auto accepted{m_store->GetBestRecord()};
    const auto durable{m_persistence->GetFinalityState().best};
    if (accepted.has_value() != durable.has_value() ||
        (accepted && accepted->metadata != *durable)) {
        return std::nullopt;
    }
    const int32_t floor{durable ? durable->statement.height
        : m_config->activation_predecessor_height};
    const CBlockIndex* predecessor{tip->GetAncestor(floor)};
    if (!predecessor || (durable &&
        predecessor->GetBlockHash() != durable->statement.block_hash)) {
        return std::nullopt;
    }
    // This is a historical import selector, not a fallback for a failed
    // live certificate. The local chain chooses the receipt before any
    // certificate supplied by a peer is considered.
    if (coverage->nHeight <= floor ||
        ClassifyHistoricalReceiptIndexRangeCached(*coverage, floor + 1) !=
            PaymentAuditContextStatus::READY) {
        return std::nullopt;
    }
    const auto selected{IndexedBTCCReceiptState(*authority)};
    const auto covered_btcc{IndexedBTCCReceiptState(*coverage)};
    const auto covered_audit{IndexedPaymentAuditReceiptState(*coverage)};
    if (!selected || selected->latest_receipt_carrier_height < 0 ||
        !covered_btcc || !covered_audit ||
        coverage->pqPaymentProbationStateHash.IsNull()) {
        return std::nullopt;
    }
    const CBlockIndex* carrier{authority->GetAncestor(
        selected->latest_receipt_carrier_height)};
    if (!carrier || !carrier->pprev ||
        carrier->pqBTCCReceiptLogicalId.IsNull() ||
        (carrier->nStatus & BLOCK_FAILED_MASK) || carrier->IsAssumedValid() ||
        !carrier->IsValid(BLOCK_VALID_SCRIPTS) || !HasFullReceiptIndexProvenance(*carrier) ||
        (carrier->pprev->nStatus & BLOCK_FAILED_MASK) || carrier->pprev->IsAssumedValid() ||
        !carrier->pprev->IsValid(BLOCK_VALID_SCRIPTS) ||
        !HasFullReceiptIndexProvenance(*carrier->pprev)) {
        return std::nullopt;
    }
    const auto before{IndexedBTCCReceiptState(*carrier->pprev)};
    const auto after{IndexedBTCCReceiptState(*carrier)};
    if (!before || !after || *after != *selected) return std::nullopt;
    const auto receipt{pq::ReconstructBTCCReceipt(
        m_genesis_hash, m_config->chainlock_schedule,
        m_config->btcc_schedule, m_config->activation_predecessor_height,
        *carrier, *before, *after, carrier->pqBTCCReceiptLogicalId)};
    if (!receipt || receipt->IsNull() ||
        receipt->chainlock_target_height <= m_config->activation_predecessor_height ||
        (durable && receipt->chainlock_target_height == floor &&
         (receipt->chainlock_logical_id != durable->logical_id ||
          receipt->chainlock_target_hash != durable->statement.block_hash ||
          receipt->accepted_cursor != durable->statement.accepted_btcc_cursor))) {
        return std::nullopt;
    }
    // The prefix can contain deferred checks after D even when B is already
    // authenticated. Only state-advancing admission may grant the D < B exception.
    const CBlockIndex* base{carrier->GetAncestor(receipt->chainlock_target_height)};
    if (!base || base->GetBlockHash() != receipt->chainlock_target_hash ||
        (base->nStatus & BLOCK_FAILED_MASK) || base->IsAssumedValid() ||
        !base->IsValid(BLOCK_VALID_SCRIPTS) || !HasFullReceiptIndexProvenance(*base)) {
        return std::nullopt;
    }
    pq::HistoricalSyncBoundary result;
    if (durable) result.durable_prior = durable->AuthorizationBase();
    result.carrier_height = carrier->nHeight;
    result.carrier_hash = carrier->GetBlockHash();
    result.receipt = *receipt;
    result.coverage_height = coverage->nHeight;
    result.coverage_hash = coverage->GetBlockHash();
    result.receipt_state = *covered_btcc;
    result.payment_audit_state = *covered_audit;
    result.probation_state_hash = coverage->pqPaymentProbationStateHash;
    if (!result.IsStructurallyValid()) return std::nullopt;
    if (!coverage_height) {
        const auto bootstrap{m_persistence->LoadHistoricalSyncBootstrap()};
        const pq::HistoricalSyncBoundary* established{m_historical_sync
            ? &m_historical_sync->boundary : (bootstrap ? &bootstrap->boundary : nullptr)};
        if (established) {
            const auto frozen_height{FrozenHistoricalCoverageForDurableRebind(*established, result)};
            if (frozen_height) {
                auto frozen{SelectPoWHistoricalSyncBoundary(*frozen_height)};
                auto rebound{*established};
                rebound.durable_prior = result.durable_prior;
                if (frozen && *frozen == rebound) return frozen;
                return std::nullopt;
            }
        }
    }
...
std::optional<int32_t> CChainLocksHandler::FrozenHistoricalCoverageForDurableRebind(
    const pq::HistoricalSyncBoundary& established,
    const pq::HistoricalSyncBoundary& selected)
{
    // Updating the real finality floor does not itself authorize a larger
    // historical interval. Reselect and verify the old E against the new D.
    if (!established.IsStructurallyValid() || !selected.IsStructurallyValid() ||
        selected.durable_prior.IsNull() ||
        selected.durable_prior.height <= established.durable_prior.height ||
        selected.durable_prior.height >= established.coverage_height ||
        established.coverage_height >= selected.coverage_height ||
        established.receipt != selected.receipt ||
        established.carrier_height != selected.carrier_height ||
        established.carrier_hash != selected.carrier_hash) return std::nullopt;
    return established.coverage_height;
}

std::optional<pq::DurableHistoricalSyncBoundary>
CChainLocksHandler::SelectDurablyCoveredHistoricalSyncBoundary() const
{
    AssertLockHeld(cs_main);
    if (!m_config || !m_store || !m_persistence || m_persistence_failed.load() ||
        !m_chainman.IsBaseBlockSyncComplete() ||
    ...
    const auto durable{m_persistence->GetFinalityState().best};
    if (!accepted || !durable || accepted->metadata != *durable ||
        !accepted->certificate || !accepted->verification_context ||
        accepted->verification_context->Authorization().admission ==
            pq::RosterAuthorizationAdmission::POW_HISTORY ||
        accepted->verification_context->Statement() != durable->statement ||
        accepted->certificate->statement != durable->statement ||
        accepted->verification_context->StatementLogicalId() != durable->logical_id ||
        durable->statement.roster_authorization_base.IsNull()) {
        return std::nullopt;
    }
    const CBlockIndex* tip{m_chainman.ActiveTip()};
    const CBlockIndex* winner{tip ? tip->GetAncestor(durable->statement.height) : nullptr};
    const auto winner_btcc{winner ? IndexedBTCCReceiptState(*winner) : std::nullopt};
    const auto winner_payment{winner ? IndexedPaymentAuditReceiptState(*winner) : std::nullopt};
    if (!winner || winner->GetBlockHash() != durable->statement.block_hash ||
        (winner->nStatus & BLOCK_FAILED_MASK) || winner->IsAssumedValid() ||
        !winner->IsValid(BLOCK_VALID_SCRIPTS) || !HasFullReceiptIndexProvenance(*winner) ||
        !winner_btcc || *winner_btcc != durable->statement.btcc_receipt_state ||
        !winner_payment || *winner_payment != durable->statement.payment_audit_receipt_state ||
        winner->pqPaymentProbationStateHash != durable->statement.payment_probation_state_hash) {
        return std::nullopt;
    }
    // Only an independently installed and fsynced successor can restore this
    // exact local record. B's own signatures never establish its old state edge.
    for (const auto& stored : m_persistence->LoadHistoricalSyncBoundaries()) {
        const auto& boundary{stored.boundary};
        const auto& certificate{stored.record.ChainLock()};
        const pq::RosterAuthorizationBaseIdentity identity{
            certificate.statement.height, certificate.statement.block_hash,
            certificate.GetLogicalId(m_genesis_hash)};
        if (!boundary.IsStructurallyValid() ||
            identity != durable->statement.roster_authorization_base ||
            boundary.receipt.chainlock_logical_id != identity.logical_id ||
            boundary.receipt.chainlock_target_height != identity.height ||
            boundary.receipt.chainlock_target_hash != identity.block_hash ||
            boundary.receipt.accepted_cursor != certificate.statement.accepted_btcc_cursor ||
            !pq::IsDurableBTCCReceiptStateMonotonic(boundary.receipt_state, *winner_btcc) ||
            !pq::IsDurablePaymentAuditStateMonotonic(
                boundary.payment_audit_state, boundary.probation_state_hash,
                *winner_payment, durable->statement.payment_probation_state_hash) ||
            boundary.coverage_height > winner->nHeight) {
            continue;
        }
        const CBlockIndex* end{winner->GetAncestor(boundary.coverage_height)};
        const CBlockIndex* carrier{winner->GetAncestor(boundary.carrier_height)};
        const auto btcc{end ? IndexedBTCCReceiptState(*end) : std::nullopt};
        const auto payment{end ? IndexedPaymentAuditReceiptState(*end) : std::nullopt};
        const auto before{carrier && carrier->pprev
            ? IndexedBTCCReceiptState(*carrier->pprev) : std::nullopt};
        const auto after{carrier ? IndexedBTCCReceiptState(*carrier) : std::nullopt};
        const auto receipt{carrier && before && after
            ? pq::ReconstructBTCCReceipt(
                  m_genesis_hash, m_config->chainlock_schedule, m_config->btcc_schedule,
                  m_config->activation_predecessor_height, *carrier,
                  *before, *after, carrier->pqBTCCReceiptLogicalId)
            : std::nullopt};
        if (!end || !carrier || end->GetBlockHash() != boundary.coverage_hash ||
            carrier->GetBlockHash() != boundary.carrier_hash ||
            carrier->pqBTCCReceiptLogicalId != identity.logical_id ||
            (end->nStatus & BLOCK_FAILED_MASK) || end->IsAssumedValid() ||
            !end->IsValid(BLOCK_VALID_SCRIPTS) || !HasFullReceiptIndexProvenance(*end) ||
            !btcc || *btcc != boundary.receipt_state ||
            !receipt || *receipt != boundary.receipt ||
            !payment || *payment != boundary.payment_audit_state ||
            end->pqPaymentProbationStateHash != boundary.probation_state_hash) {
            continue;
        }
        // C may already include a newer receipt than B. Its signed base still
        // names B exactly; fully replayed successors connect E's roots to C's.
        if (winner->nHeight > end->nHeight &&
            ClassifyHistoricalReceiptIndexRangeCached(*winner, end->nHeight + 1) !=
                PaymentAuditContextStatus::READY) {
            continue;
        }
        return stored;
...
std::optional<pq::VerifiedPoWHistoricalBoundary>
CChainLocksHandler::ValidatePoWHistoricalSyncBoundary(
    const pq::HistoricalSyncBoundary& boundary,
    const pq::FinalChainLock& certificate) const
{
    AssertLockHeld(cs_main);
    // Re-run every original selection/provenance check at the frozen active
    // ancestor. Descendant tips do not invalidate E, but a changed supporting
    // branch, receipt, state commitment, or durable predecessor still does.
    auto selected{SelectPoWHistoricalSyncBoundary(boundary.coverage_height)};
    if (!selected) {
        const auto covered{SelectDurablyCoveredHistoricalSyncBoundary()};
        if (covered && covered->boundary == boundary &&
            covered->record.ChainLock() == certificate) {
            selected = boundary;
        }
    }
    if (!selected || *selected != boundary ||
        !certificate.IsStructurallyValid() ||
        certificate.GetLogicalId(m_genesis_hash) !=
            boundary.receipt.chainlock_logical_id) {
        return std::nullopt;
    }
    ...
    const CBlockIndex* previous{target
        ? target->GetAncestor(statement.previous_chainlock_height)
        : nullptr};
    if (!target || !previous ||
        target->GetBlockHash() != statement.block_hash ||
        statement.block_hash != boundary.receipt.chainlock_target_hash ||
        previous->GetBlockHash() != statement.previous_chainlock_hash ||
        statement.height != boundary.receipt.chainlock_target_height ||
        statement.accepted_btcc_cursor != boundary.receipt.accepted_cursor ||
        target->IsAssumedValid() || !target->IsValid(BLOCK_VALID_SCRIPTS) ||
        !HasFullReceiptIndexProvenance(*target)) {
        return std::nullopt;
    }
    const auto btcc{IndexedBTCCReceiptState(*target)};
    const auto audit{IndexedPaymentAuditReceiptState(*target)};
    if (!btcc || !audit || *btcc != statement.btcc_receipt_state ||
        *audit != statement.payment_audit_receipt_state ||
        target->pqPaymentProbationStateHash !=
            statement.payment_probation_state_hash) {
        return std::nullopt;
    }
    // The receipt fixes the complete signed statement. This capability
    // substitutes PoW-history trust only for its missing authorization edge;
    // canonical roster reconstruction and all signatures remain mandatory.
    return pq::VerifiedPoWHistoricalBoundary{
        m_genesis_hash, m_config->chainlock_schedule,
        certificate.GetLogicalId(m_genesis_hash),
        pq::GetHistoricalSyncBoundaryHash(m_genesis_hash, *m_config, boundary)};
}

std::shared_ptr<const CChainLocksHandler::HistoricalSyncAuthorization>
CChainLocksHandler::GetPoWHistoricalSyncAuthorization() const
{
    AssertLockHeld(cs_main);
    const auto& imported{m_historical_sync};
    if (!imported || !m_config || !m_store || !m_persistence ||
        m_persistence_failed.load() ||
        imported->provenance_revision !=
            m_chainman.GetPQProvenanceRevocationRevision()) {
        return {};
    }
    if (!imported->record_identity.IsNull() &&
        !m_persistence->IsHistoricalSyncRecordCurrent(
            imported->boundary, imported->record_identity)) return {};
    const auto& boundary{imported->boundary};
    const CBlockIndex* tip{m_chainman.ActiveTip()};
    const CBlockIndex* end{tip && tip->nHeight >= boundary.coverage_height
        ? tip->GetAncestor(boundary.coverage_height) : nullptr};
    const CBlockIndex* carrier{end
        ? end->GetAncestor(boundary.carrier_height) : nullptr};
    if (!end || !carrier ||
        end->GetBlockHash() != boundary.coverage_hash ||
        carrier->GetBlockHash() != boundary.carrier_hash ||
        carrier->pqBTCCReceiptLogicalId !=
            boundary.receipt.chainlock_logical_id ||
        (end->nStatus & BLOCK_FAILED_MASK) ||
        !HasFullReceiptIndexProvenance(*end)) {
        return {};
    }
    const auto btcc{IndexedBTCCReceiptState(*end)};
    const auto payment{IndexedPaymentAuditReceiptState(*end)};
    if (!btcc || !payment || *btcc != boundary.receipt_state ||
        *payment != boundary.payment_audit_state ||
        end->pqPaymentProbationStateHash != boundary.probation_state_hash) {
        return {};
    }
    std::optional<pq::FinalChainLockRecordMetadata> durable;
    ...
    if (accepted.has_value() != durable.has_value() ||
        (accepted && accepted->metadata != *durable)) return {};
    if (boundary.coverage_height > (durable ? durable->statement.height
                                          : m_config->activation_predecessor_height) &&
        boundary.durable_prior != (durable ? durable->AuthorizationBase()
                                          : pq::RosterAuthorizationBaseIdentity{})) return {};
    if (!boundary.durable_prior.IsNull()) {
        const CBlockIndex* prior{tip->GetAncestor(
            boundary.durable_prior.height)};
        if (!durable || !prior || prior->GetBlockHash() !=
                boundary.durable_prior.block_hash ||
            durable->statement.height < boundary.durable_prior.height ||
            (durable->statement.height == boundary.durable_prior.height &&
             durable->AuthorizationBase() != boundary.durable_prior)) {
            return {};
        }
    }
    if (durable) {
    ...
    std::optional<pq::VerifiedHistoricalSyncSuccessor>& proof) const
{
    AssertLockHeld(cs_main);
    proof.reset();
    if (!m_historical_sync ||
        chainlock.statement.roster_authorization_base !=
            m_historical_sync->base.metadata.AuthorizationBase()) return true;
    const auto imported{GetPoWHistoricalSyncAuthorization()};
    ...
    if (durable && imported->base.metadata.statement.height <= durable->statement.height) {
        ...
        if (ordinary && ordinary->metadata.statement == base.statement &&
            ...
            ordinary->verification_context->Authorization().admission !=
                pq::RosterAuthorizationAdmission::POW_HISTORY) {
            // Ordinary B already owns the new edge's dependency. Prefix-only
            // evidence must not rotate protected serving records unnecessarily.
            return true;
        }
    }
    const auto precommit{m_persistence->LoadRosterRecoveryPrecommit()};
    uint64_t revision{0};
    const auto bootstrap{m_persistence->LoadHistoricalSyncBootstrap(&revision)};
    ...
    if (!target || target->nHeight != chainlock.statement.height ||
        ...
        (!durable && chainlock.statement.roster_transition ==
            pq::RosterAuthorizationTransitionKind::RECOVER &&
         !m_chainman.ActiveChain().Contains(target)) ||
        target->nHeight <= boundary.coverage_height ||
        target->GetAncestor(boundary.coverage_height)->GetBlockHash() !=
            boundary.coverage_hash ||
        (durable ? durable->AuthorizationBase()
                 : pq::RosterAuthorizationBaseIdentity{}) != boundary.durable_prior ||
        chainlock.statement.roster_transition ==
            pq::RosterAuthorizationTransitionKind::INITIALIZE) return false;
    std::array<uint256, 2> covered_serving_records{};
    uint64_t serving_revision{0};
    const auto serving{m_persistence->LoadHistoricalSyncBoundaries(&serving_revision)};
    if (serving_revision != revision) return false;
    for (std::size_t i{0}; i < serving.size(); ++i) {
        const auto& retained{serving[i].boundary};
        const CBlockIndex* carrier{target->GetAncestor(retained.carrier_height)};
        const CBlockIndex* end{target->GetAncestor(retained.coverage_height)};
        if (!carrier || !end || carrier->GetBlockHash() != retained.carrier_hash ||
            end->GetBlockHash() != retained.coverage_hash) return false;
        covered_serving_records[i] = serving[i].record.RecordIdentity();
    }
    // The store rechecks the winner/context before handing this exact snapshot
    // to persistence; persistence then CAS-checks every role in the same batch.
    proof = pq::VerifiedHistoricalSyncSuccessor{
        boundary, chainlock.GetLogicalId(m_genesis_hash), precommit, revision,
        promote_bootstrap ? imported->record_identity : uint256{},
        covered_serving_records};
    return true;
}

void CChainLocksHandler::CompleteHistoricalSyncSuccessor(
    const pq::VerifiedHistoricalSyncSuccessor& proof,
    const std::shared_ptr<const HistoricalSyncAuthorization>& imported)
{
    AssertLockHeld(cs_main);
    RefreshHistoricalSyncServableRecords();
    if (proof.BootstrapRecordIdentity().IsNull() || !imported ||
        imported->record_identity != proof.BootstrapRecordIdentity() ||
        imported->boundary != proof.Boundary() ||
        (m_historical_sync && m_historical_sync != imported)) return;
    const auto accepted{m_store->GetBestRecord()};
    const auto durable{m_persistence->GetFinalityState().best};
    uint64_t revision{0};
    const auto serving{m_persistence->LoadHistoricalSyncBoundaries(&revision)};
    if (accepted && durable && accepted->metadata == *durable &&
        accepted->metadata.logical_id == proof.CandidateLogicalId() &&
        imported->base.certificate) {
        for (const auto& retained : serving) {
            if (retained.boundary != imported->boundary ||
                retained.record.ChainLock() != *imported->base.certificate) continue;
            // Promotion changes only B's durable role. Preserve its verified
            // PoW-history context and frozen endpoint, never mint live B authority.
            auto rebound{*imported};
            rebound.record_identity = retained.record.RecordIdentity();
            rebound.base.base_revision = revision;
            m_historical_sync = std::make_shared<const HistoricalSyncAuthorization>(std::move(rebound));
            if (GetPoWHistoricalSyncAuthorization()) return;
            break;
        }
    }
    RevokeReorgedHistoricalSyncAuthorizationLocked();
...
bool CChainLocksHandler::IsPoWHistoricalPrefixCovered(
    const CBlockIndex& index) const
{
    AssertLockHeld(cs_main);
    const auto imported{GetPoWHistoricalSyncAuthorization()};
    if (!imported || index.nHeight > imported->boundary.coverage_height ||
        index.nHeight <= m_config->activation_predecessor_height) {
        return false;
    }
    const CBlockIndex* end{m_chainman.ActiveChain()[
        imported->boundary.coverage_height]};
    return end && end->GetAncestor(index.nHeight) == &index;
}

std::optional<bool> CChainLocksHandler::ProcessPoWHistoricalSyncCertificate(
    NodeId from, const pq::FinalChainLock& certificate,
    BlockValidationState& state, bool* peer_fault)
{
    std::optional<pq::HistoricalSyncBoundary> boundary;
    std::optional<pq::VerifiedPoWHistoricalBoundary> proof;
    const uint256 logical_id{certificate.GetLogicalId(m_genesis_hash)};
    const uint256 witness_id{certificate.GetWitnessId(m_genesis_hash)};
    uint256 restored_record_identity;
    uint64_t revision{0};
    LiveSigningValidationFrontier signing_frontier;
    {
        LOCK(m_context_build_mutex);
        signing_frontier = m_live_signing_validation_frontier;
    }
    {
        LOCK(cs_main);
        RevokeReorgedHistoricalSyncAuthorizationLocked();
        boundary = SelectPoWHistoricalSyncBoundary();
        if (!boundary && from == -1) {
            const auto covered{SelectDurablyCoveredHistoricalSyncBoundary()};
            if (covered && covered->record.ChainLock() == certificate) {
                boundary = covered->boundary;
                restored_record_identity = covered->record.RecordIdentity();
            }
        }
        if (!boundary || boundary->receipt.chainlock_logical_id != logical_id) {
            return std::nullopt;
        }
        const auto previous{GetPoWHistoricalSyncAuthorization()};
        if (previous && previous->boundary.durable_prior == boundary->durable_prior &&
            previous->boundary.receipt == boundary->receipt &&
            previous->boundary.carrier_hash == boundary->carrier_hash &&
            !ShouldExtendPoWHistoricalCoverageForGovernance(
                previous->boundary, *boundary, signing_frontier,
                m_chainman.ActiveChain(), m_chainman.GetPQProvenanceRevocationRevision(),
                m_genesis_hash, *m_config)) {
            // An alternate witness for the same statement cannot replace the
            // verified capability or move its frozen coverage endpoint.
            CompletePeerResponse(from, logical_id);
            return true;
        }
        proof = ValidatePoWHistoricalSyncBoundary(*boundary, certificate);
        if (!proof) {
            CompletePeerResponse(from, logical_id);
            return state.Error("pq-clsig-historical-boundary-unavailable");
        }
        (void)m_persistence->LoadHistoricalSyncBoundaries(&revision);
    }
    ScopedFinalitySnapshotVerificationRetention retention{
        deterministicMNManager.get()};
    const auto cache{GetQuorumRosterCache()};
    if (!cache) return state.Error("pq-clsig-historical-rosters-unavailable");
    pq::VerifiedRosterSetPtr rosters;
    {
        LOCK(cs_main);
        const CBlockIndex* target{m_chainman.ActiveChain()[
            certificate.statement.height]};
        if (!target || !ValidatePoWHistoricalSyncBoundary(*boundary, certificate)) {
            return state.Error("pq-clsig-historical-boundary-changed");
        }
        rosters = cache->GetVerifiedActiveNoPublish(
            certificate.statement.height, *target,
            certificate.statement.roster_beacons.active);
    }
    if (!rosters) return state.Error("pq-clsig-historical-rosters-unavailable");
    const auto context{pq::PreparedChainLockContext::CreateFromPoWHistory(
        m_config->chainlock_schedule, certificate.statement, rosters, *proof)};
    auto checks{context ? pq::PrepareFinalChainLockVerification(certificate, *context)
                        : std::nullopt};
    bool valid{false};
    if (checks) {
        LOCK(m_verification_mutex);
        valid = m_verifier.VerifyChecks(std::move(checks->checks));
    }
    if (!valid) {
        m_store->RejectWitness(certificate);
        if (peer_fault) *peer_fault = true;
        FailPeerResponse(from, logical_id);
        return state.Invalid(BlockValidationResult::BLOCK_CHAINLOCK,
                             "pq-clsig-invalid-historical-signatures");
    }
    const bool published{m_chainman.ActiveChainstate().RunWithStableActiveChain([&] {
        LOCK(cs_main);
        if (!ValidatePoWHistoricalSyncBoundary(*boundary, certificate)) return false;
        const bool restoring{!restored_record_identity.IsNull()};
        if (restoring) {
            const auto covered{SelectDurablyCoveredHistoricalSyncBoundary()};
            if (!covered || covered->boundary != *boundary ||
                covered->record.RecordIdentity() != restored_record_identity ||
                covered->record.ChainLock() != certificate) return false;
        }
        pq::RecoveryUniverseCapsulePtr universe;
        if (!CaptureRecoveryUniverseForDurableCandidate(certificate, universe)) return false;
        const auto publication{BeginChainLockAuxiliarySnapshotPublication()};
        if (!publication) return false;
        pq::ChainLockPersistenceError error{pq::ChainLockPersistenceError::NONE};
        const bool flushed{restoring ||
            (m_chainman.m_blockman.FlushChainstateBlockFile(boundary->coverage_height) &&
             FlushBTCCIndexStateForDurableAcceptanceLocked(certificate))};
        // Restoration retains the original fsynced provenance and cannot turn
        // B into an ordinary authorization archive or advance the durable best.
        if (!restoring && flushed) m_historical_sync.reset();
        const bool persisted{flushed && (restoring ||
            m_persistence->PersistHistoricalSyncBootstrap(
                certificate, context, *boundary, revision, universe, &error))};
        if (!CompleteChainLockAuxiliarySnapshotPublication(*publication)) return false;
        if (!flushed || error == pq::ChainLockPersistenceError::IO_FAILURE) {
            m_persistence_failed.store(true);
            DisableShareAdmission();
        ...
        const auto saved{m_persistence->LoadHistoricalSyncBootstrap(&revision)};
        if (!restoring && (!saved || saved->boundary != *boundary ||
                saved->record.ChainLock() != certificate)) return false;
        const auto shared_certificate{std::make_shared<const pq::FinalChainLock>(certificate)};
        m_historical_sync = std::make_shared<const HistoricalSyncAuthorization>(
            HistoricalSyncAuthorization{*boundary,
                pq::VerifiedRosterAuthorizationBaseView{
                    revision,
                    {logical_id, witness_id, certificate.statement},
                    shared_certificate, context},
                m_chainman.GetPQProvenanceRevocationRevision(),
                restoring ? restored_record_identity : saved->record.RecordIdentity()});
        RefreshHistoricalSyncServableRecords();
        return true;
    })};
    CompletePeerResponse(from, logical_id);
    if (!published) return state.Error("pq-clsig-historical-publication-deferred");
    ForgetAllRequests(logical_id);
    ClearNeededBTCCCertificate(logical_id);
    LogPrint(BCLog::CHAINLOCKS,
             "CChainLocksHandler::%s imported PoW historical authority %s "
             "through %d without advancing finality\n", __func__,
             logical_id.ToString(), boundary->coverage_height);
    return true;
}

bool CChainLocksHandler::RefreshPoWHistoricalSyncBoundary()
{
    if (!IsChainLockVerificationAvailable()) return false;
    std::optional<pq::HistoricalSyncBoundary> boundary;
    std::shared_ptr<const pq::FinalChainLock> local;
    LiveSigningValidationFrontier signing_frontier;
    {
        LOCK(m_context_build_mutex);
        signing_frontier = m_live_signing_validation_frontier;
    }
    {
        ...
        boundary = SelectPoWHistoricalSyncBoundary();
        if (!boundary) {
            if (GetPoWHistoricalSyncAuthorization()) return false;
            const auto covered{SelectDurablyCoveredHistoricalSyncBoundary()};
            if (covered) {
                boundary = covered->boundary;
                local = std::make_shared<const pq::FinalChainLock>(covered->record.ChainLock());
            }
        }
        if (!boundary) return false;
        const auto revalidation{m_historical_sync_revalidation_input.load()};
        const bool has_revalidation_input{revalidation && revalidation->certificate &&
            revalidation->certificate->GetLogicalId(m_genesis_hash) ==
                boundary->receipt.chainlock_logical_id};
        if (!local && has_revalidation_input) local = revalidation->certificate;
        const auto imported{GetPoWHistoricalSyncAuthorization()};
        // Descendant blocks do not create a new historical trust decision.
        // Keep E frozen so budgeted validation can finish across scheduler ticks.
        if (imported && imported->boundary.durable_prior == boundary->durable_prior &&
            imported->boundary.receipt == boundary->receipt &&
            imported->boundary.carrier_hash == boundary->carrier_hash &&
            !ShouldExtendPoWHistoricalCoverageForGovernance(
                imported->boundary, *boundary, signing_frontier,
                m_chainman.ActiveChain(), m_chainman.GetPQProvenanceRevocationRevision(),
                m_genesis_hash, *m_config)) {
            if (revalidation) {
                m_historical_sync_revalidation_input.store({});
        ...
        const auto now{GetTime<std::chrono::microseconds>()};
        if (!has_revalidation_input &&
            m_historical_sync_requested == boundary->receipt.chainlock_logical_id &&
            m_historical_sync_last_request.count() &&
            now - m_historical_sync_last_request < std::chrono::seconds{60}) {
            // Do not reauthorize an upload still inside the network's
            // certificate-response deadline. Its INV can arrive late.
            return true;
        }
        m_historical_sync_requested = boundary->receipt.chainlock_logical_id;
        m_historical_sync_last_request = now;
        if (!local) local = m_store->GetServableByLogicalId(m_historical_sync_requested);
        if (!local) {
            for (const auto& saved : m_historical_sync_servable) {
                if (saved && saved->GetLogicalId(m_genesis_hash) ==
                        m_historical_sync_requested) {
                    local = saved;
                    break;
                }
            }
        ...
        if (!local) {
            for (const auto& saved : m_persistence->LoadHistoricalSyncBoundaries()) {
                if (saved.record.ChainLock().GetLogicalId(m_genesis_hash) ==
                    m_historical_sync_requested) {
                    local = std::make_shared<const pq::FinalChainLock>(saved.record.ChainLock());
                    break;
                }
            }
        }
        if (!local) {
            const auto bootstrap{m_persistence->LoadHistoricalSyncBootstrap()};
            if (bootstrap && bootstrap->record.ChainLock().GetLogicalId(m_genesis_hash) ==
                    m_historical_sync_requested) {
                local = std::make_shared<const pq::FinalChainLock>(bootstrap->record.ChainLock());
            }
        }
    }
    if (local) {
        BlockValidationState state;
        (void)ProcessNewChainLock(-1, *local, state);
        LOCK(cs_main);
        if (m_historical_sync_revalidation_input.load() &&
            GetPoWHistoricalSyncAuthorization()) {
            m_historical_sync_revalidation_input.store({});
            UpdateDurableChainLockAuxiliaryRetention();
        }
    } else {
        m_connman.ForEachNode([&](CNode* node) {
            ...
            const CNetMsgMaker maker{node->GetCommonVersion()};
...
void CChainLocksHandler::RequestCatchupChainLock()
{
    ...
    {
        LOCK(cs_main);
        if (!GetPoWHistoricalSyncAuthorization() &&
    ...
    std::optional<uint256> exact_retry;
    {
        LOCK(cs_main);
        const CBlockIndex* tip{m_chainman.ActiveTip()};
        const auto latest{tip == nullptr
        ...
        const int32_t local_height{
            best ? best->metadata.statement.height
                 : m_config->activation_predecessor_height};
        if (!latest || *latest <= local_height) return;
        if (m_catchup_candidate_hint &&
            (m_catchup_candidate_hint->height != *latest ||
             tip->GetAncestor(*latest)->GetBlockHash() != m_catchup_candidate_hint->block_hash)) {
            m_catchup_candidate_hint.reset();
            m_catchup_retry_requested = false;
        }
        if (!m_catchup_retry_requested &&
            GetPoWHistoricalSyncAuthorization() && m_catchup_candidate_hint &&
            m_catchup_candidate_hint->height == *latest &&
            tip->GetAncestor(*latest)->GetBlockHash() == m_catchup_candidate_hint->block_hash) {
            exact_retry = m_catchup_candidate_hint->logical_id;
            m_catchup_retry_requested = true;
        }
    ...
    // Empty latest queries intentionally suppress already announced inventory.
    // After B arrives, use the existing bounded by-ID retry for the earlier C.
    if (exact_retry) {
        m_connman.ForEachNode([&](CNode* node) {
            if (!SupportsPQChainLocks(node->GetCommonVersion())) return;
            const CNetMsgMaker maker{node->GetCommonVersion()};
            m_connman.PushMessage(node, maker.Make(NetMsgType::GETCLSIG, *exact_retry));
        });
    } else {
...
uint256 CChainLocksHandler::GetPaymentAuditReplaySourceToken() const
{
    AssertLockHeld(cs_main);
    AssertLockNotHeld(m_lookup_mutex);
    AssertLockNotHeld(m_btcc_preseal_mutex);
    ...
    const auto durable{m_persistence->GetFinalityState()};
    pq::PaymentAuditPresealState state;
    {
        LOCK(m_btcc_preseal_mutex);
        state = m_payment_audit_preseal_state;
    }
    if (state.IsEmpty()) return {};
    CHashWriter writer{SER_GETHASH, 0};
    writer << std::string{"SYS_PQ_PAYMENT_AUDIT_BOUNDED_REPLAY_V1"}
           << PaymentAuditPresealStateToken(state)
           << m_chainman.GetPQProvenanceRevocationRevision()
           << roster_generation
           << deterministicMNManager->PaymentProbationStateViewGeneration()
           << PaymentAuditCheckpointToken(m_payment_audit_store->GetPruneCheckpoint())
           << durable.best.has_value();
    if (durable.best) {
...
bool CChainLocksHandler::IsPaymentAuditReplayFloorStatementValid(
    const pq::ChainLockStatement& statement, const CBlockIndex& index) const
{
    AssertLockHeld(cs_main);
    const auto btcc{IndexedBTCCReceiptState(index)};
    const auto payment{IndexedPaymentAuditReceiptState(index)};
    return statement.height == index.nHeight && statement.block_hash == index.GetBlockHash() &&
           m_chainman.ActiveChain()[index.nHeight] == &index &&
           !index.IsAssumedValid() && index.IsValid(BLOCK_VALID_SCRIPTS) &&
           !(index.nStatus & (BLOCK_FAILED_MASK | BLOCK_CONFLICT_CHAINLOCK)) &&
           HasFullReceiptIndexProvenance(index) && btcc && payment &&
           *btcc == statement.btcc_receipt_state &&
           *payment == statement.payment_audit_receipt_state &&
           index.pqPaymentProbationStateHash == statement.payment_probation_state_hash;
}

bool CChainLocksHandler::IsPaymentAuditReplaySealFloorValid(
    const pq::VerifiedRosterAuthorizationBaseView& seal,
    const CBlockIndex& index) const
{
    AssertLockHeld(cs_main);
    if (!m_config || !seal.certificate || !seal.verification_context ||
        !seal.metadata.IsInternallyConsistent(m_genesis_hash)) return false;
    const auto& statement{seal.metadata.statement};
    const auto& context{*seal.verification_context};
    // The typed store view already crossed full signature verification.
    // Borrow that immutable fact for replay without promoting finality or GC.
    return seal.certificate->statement == statement &&
           context.GenesisHash() == m_genesis_hash &&
           context.Schedule() == m_config->chainlock_schedule &&
           context.Statement() == statement &&
           context.StatementLogicalId() == seal.metadata.logical_id &&
           context.Authorization().admission != pq::RosterAuthorizationAdmission::POW_HISTORY &&
           pq::IsSigningRosterAuthorizationMask(context.AuthorizationMask()) &&
           (seal.certificate->selected_quorum_mask & context.AuthorizationMask()) ==
               seal.certificate->selected_quorum_mask &&
           IsPaymentAuditReplayFloorStatementValid(statement, index);
}

bool CChainLocksHandler::IsPaymentAuditReplayValidationCurrent() const
{
    AssertLockHeld(cs_main);
    const auto& runtime{m_payment_audit_replay_validation};
    if (runtime.source_token.IsNull() ||
        runtime.source_token != GetPaymentAuditReplaySourceToken()) return false;
    const CChain& chain{m_chainman.ActiveChain()};
    const CBlockIndex* floor{chain[runtime.floor_height]};
    const CBlockIndex* through{chain[runtime.frontier.ValidatedThroughHeight()]};
    if (runtime.historical_receipt) {
        const auto& receipt{runtime.historical_receipt->m_receipt};
        const CBlockIndex* carrier{chain[receipt.carrier_height]};
        if (carrier == nullptr || GetPaymentAuditHistoricalReplayTransition(receipt, *carrier) !=
                runtime.historical_receipt) return false;
    }
    return floor != nullptr && through != nullptr &&
           floor->GetBlockHash() == runtime.floor_hash &&
...
bool CChainLocksHandler::IsPaymentAuditReplayAuthenticated(
    const CBlockIndex& index) const
{
    AssertLockHeld(cs_main);
    if (IsPaymentAuditPrefixAuthenticated(index)) return true;
    const auto& runtime{m_payment_audit_replay_validation};
...
std::optional<CChainLocksHandler::PendingPaymentAuditReceiptDependency>
CChainLocksHandler::GetPaymentAuditReplayDependency() const
{
    AssertLockHeld(cs_main);
    AssertLockNotHeld(m_btcc_preseal_mutex);
    const auto& runtime{m_payment_audit_replay_validation};
    if (!runtime.dependency || !IsPaymentAuditReplayValidationCurrent()) return std::nullopt;
    const auto& dependency{*runtime.dependency};
    const CBlockIndex* carrier{m_chainman.ActiveChain()[dependency.receipt.carrier_height]};
    bool exact_terminal{false};
    if (runtime.terminal_probe) {
        LOCK(m_btcc_preseal_mutex);
        const auto matches = [&](const auto& marker) {
            return marker && marker->terminal_carrier_hash == dependency.carrier_hash &&
                   marker->terminal_carrier_height == dependency.receipt.carrier_height &&
                   marker->terminal_receipt == dependency.receipt;
        };
        exact_terminal = matches(m_payment_audit_preseal_state.active) ||
                         matches(m_payment_audit_preseal_state.prospective);
    }
    if (dependency.source != PaymentAuditReceiptDependencySource::PRESEAL_REPLAY ||
        dependency.source_token != runtime.source_token || dependency.receipt.IsNull() ||
        carrier == nullptr || carrier->pprev == nullptr ||
        (runtime.terminal_probe ? !exact_terminal :
            carrier->nHeight != runtime.frontier.ValidatedThroughHeight() + 1) ||
        carrier->GetBlockHash() != dependency.carrier_hash ||
        carrier->pprev->GetBlockHash() != dependency.carrier_parent_hash ||
        carrier->IsAssumedValid() || !carrier->IsValid(BLOCK_VALID_SCRIPTS) ||
        (carrier->nStatus & (BLOCK_FAILED_MASK | BLOCK_CONFLICT_CHAINLOCK)) ||
        !HasFullReceiptIndexProvenance(*carrier)) return std::nullopt;
    return dependency;
}

void CChainLocksHandler::AdvancePaymentAuditPresealReplayValidation(
    const CBlockIndex& active_tip, const pq::PaymentAuditPresealState& state)
{
    AssertLockHeld(cs_main);
    AssertLockNotHeld(m_btcc_preseal_mutex);
    AssertLockNotHeld(m_lookup_mutex);
    AssertLockNotHeld(m_verification_mutex);
    auto& runtime{m_payment_audit_replay_validation};
    runtime.dependency.reset();
    runtime.terminal_probe = false;
    {
        LOCK(m_btcc_preseal_mutex);
        if (state != m_payment_audit_preseal_state) {
            runtime = {};
            return;
        }
    }
    const uint256 source{GetPaymentAuditReplaySourceToken()};
    if (source.IsNull()) {
        runtime = {};
        return;
    }
    ...
    if (runtime.historical_receipt) {
        ...
        const CBlockIndex* carrier{chain[receipt.carrier_height]};
        if (carrier == nullptr || GetPaymentAuditHistoricalReplayTransition(receipt, *carrier) !=
                runtime.historical_receipt) {
            // Revoking historical proof also revokes all progress that used it.
            runtime.frontier = {};
            runtime.historical_receipt.reset();
        }
    }
    const auto usable = [](const CBlockIndex* index)
        EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
        return index != nullptr && !index->IsAssumedValid() &&
               index->IsValid(BLOCK_VALID_SCRIPTS) &&
               !(index->nStatus & (BLOCK_FAILED_MASK | BLOCK_CONFLICT_CHAINLOCK)) &&
               HasFullReceiptIndexProvenance(*index);
    };
    const CBlockIndex* floor{nullptr};
    const pq::PaymentAuditPresealMarker* latest_marker{nullptr};
    const auto anchor = [&](const auto& candidate)
        EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
        if (!candidate) return true;
        const CBlockIndex* terminal{chain[candidate->terminal_carrier_height]};
        if (terminal == nullptr || terminal->GetBlockHash() != candidate->terminal_carrier_hash) {
            return true;
        }
        const CBlockIndex* earliest{chain[candidate->earliest_carrier_height]};
        if (!candidate->IsStructurallyValid() || !usable(earliest) ||
            !usable(earliest->pprev) || !usable(terminal) || !usable(terminal->pprev) ||
            earliest->GetBlockHash() != candidate->earliest_carrier_hash ||
            IndexedPaymentAuditReceiptState(*earliest->pprev) != candidate->predecessor_receipt_state ||
            earliest->pprev->pqPaymentProbationStateHash != candidate->predecessor_probation_state_hash) {
            return false;
        }
        const auto previous{IndexedPaymentAuditReceiptState(*terminal->pprev)};
        const auto applied{previous ? pq::ApplyPaymentAuditReceipt(
            m_genesis_hash, *previous, candidate->terminal_receipt) : std::nullopt};
        if (!applied || applied != IndexedPaymentAuditReceiptState(*terminal) ||
            candidate->terminal_receipt.next_probation_state_hash != terminal->pqPaymentProbationStateHash) {
            return false;
        }
        if (floor == nullptr || earliest->pprev->nHeight < floor->nHeight) floor = earliest->pprev;
        if (latest_marker == nullptr || terminal->nHeight > latest_marker->terminal_carrier_height) {
            latest_marker = &*candidate;
        }
        return true;
    };
    if (!anchor(state.active) || !anchor(state.prospective) || floor == nullptr) {
        runtime = {};
        return;
    }
    // Existing finality covers a pruned prefix. A fully verified ordinary
    // audit seal also authenticates its cumulative state for replay only.
    bool checkpoint_floor{false};
    const auto checkpoint{m_payment_audit_store->GetPruneCheckpoint()};
    if (checkpoint) {
        const CBlockIndex* covered{chain[std::min(active_tip.nHeight, checkpoint->covered_through_height)]};
        if (usable(covered) && covered->nHeight > floor->nHeight &&
            IsPaymentAuditPrefixAuthenticated(*covered)) {
            floor = covered;
    ...
    const auto& terminal_receipt{latest_marker->terminal_receipt};
    const CBlockIndex* terminal{chain[latest_marker->terminal_carrier_height]};
    const CBlockIndex* seal_index{chain[terminal_receipt.seal_height]};
    const bool exact_seal{usable(seal_index) &&
        seal_index->GetBlockHash() == terminal_receipt.seal_block_hash &&
        terminal->nHeight >= seal_index->nHeight &&
        terminal->GetAncestor(seal_index->nHeight) == seal_index};
    if (exact_seal && seal_index->nHeight > floor->nHeight) {
        const auto select = [&](const pq::VerifiedRosterAuthorizationBaseView& view)
            EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
            if (!IsPaymentAuditReplaySealFloorValid(view, *seal_index)) return false;
            seal_floor = view;
            return true;
        };
        if (!runtime.seal_floor || !select(*runtime.seal_floor)) {
            for (const auto& view : m_store->GetVerifiedRosterAuthorizationBasesForTarget(
                     seal_index->nHeight, seal_index->GetBlockHash())) {
                if (select(view)) break;
            }
        }
        if (seal_floor) {
            floor = seal_index;
    ...
    if (exact_seal && seal_index->nHeight > floor->nHeight) {
        VerifiedPaymentAuditReceiptTransitionPtr verified{
            GetPaymentAuditHistoricalReplayTransition(terminal_receipt, *terminal)};
        const bool historical{verified != nullptr};
        if (!verified && runtime.source_token == source && runtime.audit_floor &&
            !runtime.audit_floor_historical &&
            runtime.audit_floor->m_receipt == terminal_receipt &&
            runtime.audit_floor->m_carrier_parent_hash == terminal->pprev->GetBlockHash() &&
            runtime.audit_floor->m_parent_probation_state_hash == terminal->pprev->pqPaymentProbationStateHash) {
            verified = runtime.audit_floor;
        } else if (!verified && CheckPaymentAuditReceiptCertificate(terminal_receipt, *terminal, verified) !=
                       PaymentAuditReceiptCertificateStatus::VERIFIED) {
            verified.reset();
        }
        if (verified &&
            IsPaymentAuditReplayFloorStatementValid(verified->m_statement.seal_statement, *seal_index)) {
            // The verified audit authenticates the embedded seal. Historical
            // proof remains separately scoped and is rechecked before replay.
            audit_floor = std::move(verified);
    ...
    runtime.floor_height = floor->nHeight;
    ...
    static constexpr std::size_t MAX_PAYMENT_AUDIT_REPLAY_VALIDATION_BLOCKS{256};
    const auto plan{runtime.frontier.Plan(chain, active_tip, floor->nHeight,
        floor->GetBlockHash(), source, MAX_PAYMENT_AUDIT_REPLAY_VALIDATION_BLOCKS)};
    if (plan.status == BoundedActiveRangeStatus::INVALID) {
        runtime = {};
        return;
    }
    if (plan.status != BoundedActiveRangeStatus::WORK) return;
    const auto publish_missing = [&](const pq::PaymentAuditReceipt* missing,
                                     const CBlockIndex* missing_carrier)
        EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
        // Ask for the last package first: its ordinary seal can cover audit
        // proofs that peers legitimately pruned. Local proofs may still make
        // bounded progress while that exact seal dependency is fetched.
        const bool probe{exact_seal && seal_index->nHeight > floor->nHeight};
        const pq::PaymentAuditReceipt* receipt{probe ? &terminal_receipt : missing};
        const CBlockIndex* carrier{probe ? terminal : missing_carrier};
        if (receipt == nullptr || carrier == nullptr || carrier->pprev == nullptr) return;
        runtime.terminal_probe = probe;
        runtime.dependency = PendingPaymentAuditReceiptDependency{
            *receipt, carrier->GetBlockHash(), carrier->pprev->GetBlockHash(),
            PaymentAuditReceiptDependencySource::PRESEAL_REPLAY, source};
    };
    const pq::PaymentAuditScheduleConfig schedule{m_config->chainlock_schedule, m_config->btcc_schedule};
    for (int32_t height{plan.first_height}; height <= plan.last_height; ++height) {
        const CBlockIndex* index{chain[height]};
        pq::PaymentAuditReceipt receipt;
        const auto verify = [&]() EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
            if (!usable(index) || !usable(index->pprev)) return PaymentAuditReceiptCertificateStatus::LOCAL_ERROR;
            const auto previous{IndexedPaymentAuditReceiptState(*index->pprev)};
            const auto indexed{IndexedPaymentAuditReceiptState(*index)};
            if (!previous || !indexed) return PaymentAuditReceiptCertificateStatus::LOCAL_ERROR;
            if (pq::PaymentAuditReceiptSlotEpoch(schedule, height)) {
                CBlock block;
                if (!m_chainman.m_blockman.ReadBlockFromDisk(block, *index) ||
                    block.GetHash() != index->GetBlockHash() || !ExtractPaymentAuditReceipt(block, receipt)) {
                    return PaymentAuditReceiptCertificateStatus::LOCAL_ERROR;
                }
            }
            if (!receipt.IsNull() && ClassifyPaymentAuditReceiptCarrierContext(receipt, *index, schedule) !=
                    PaymentAuditContextStatus::READY) return PaymentAuditReceiptCertificateStatus::INVALID;
            const auto applied{pq::ApplyPaymentAuditReceipt(m_genesis_hash, *previous, receipt)};
            const uint256& probation_hash{receipt.IsNull() ? index->pprev->pqPaymentProbationStateHash
                                                        : receipt.next_probation_state_hash};
            if (!applied || *applied != *indexed || probation_hash != index->pqPaymentProbationStateHash) {
                return PaymentAuditReceiptCertificateStatus::LOCAL_ERROR;
            }
            if (receipt.IsNull()) return PaymentAuditReceiptCertificateStatus::VERIFIED;
            VerifiedPaymentAuditReceiptTransitionPtr verified{
                GetPaymentAuditHistoricalReplayTransition(receipt, *index)};
            if (verified) runtime.historical_receipt = verified;
            const bool held_terminal{!runtime.audit_floor_historical && runtime.audit_floor &&
                runtime.audit_floor->m_receipt == receipt &&
                runtime.audit_floor->m_carrier_parent_hash == index->pprev->GetBlockHash() &&
                runtime.audit_floor->m_parent_probation_state_hash == index->pprev->pqPaymentProbationStateHash};
            if (!verified && held_terminal) verified = runtime.audit_floor;
            const auto status{verified ? PaymentAuditReceiptCertificateStatus::VERIFIED :
                CheckPaymentAuditReceiptCertificate(receipt, *index, verified)};
            if (status != PaymentAuditReceiptCertificateStatus::VERIFIED) return status;
            const auto* transition{GetVerifiedPaymentAuditReceiptTransition(verified)};
            const pq::PQPaymentAuditReceiptIdentity expected{receipt.epoch, receipt.carrier_height, receipt.result_hash};
            return transition != nullptr && transition->PreviousStateHash() == index->pprev->pqPaymentProbationStateHash &&
                   transition->AppliedReceipt() == expected && transition->Result().StateHash() == probation_hash
                ? PaymentAuditReceiptCertificateStatus::VERIFIED : PaymentAuditReceiptCertificateStatus::LOCAL_ERROR;
        };
        const auto status{verify()};
        if (status != PaymentAuditReceiptCertificateStatus::VERIFIED) {
            if (!runtime.frontier.CommitThrough(chain, height - 1)) {
                runtime = {};
                return;
            }
            if (status == PaymentAuditReceiptCertificateStatus::MISSING && !receipt.IsNull() && index->pprev) {
                publish_missing(&receipt, index);
            } else {
                // A later authenticated seal may cover an old roster or
                // probation dependency that is no longer retained locally.
                publish_missing(nullptr, nullptr);
            }
            return;
        }
    }
    if (!runtime.frontier.CommitThrough(chain, plan.last_height) ||
        source != GetPaymentAuditReplaySourceToken()) {
        runtime = {};
    } else if (!runtime.frontier.IsComplete(active_tip)) {
        publish_missing(nullptr, nullptr);
    }
}
...
void CChainLocksHandler::MaybeReplayPaymentAuditPreseal()
{
    ...
    uint256 replay_source;
    {
        LOCK(cs_main);
        {
            LOCK(m_btcc_preseal_mutex);
            if (durable != m_payment_audit_preseal_state) return;
        }
        const CBlockIndex* active_tip{m_chainman.ActiveTip()};
        ...
        // Make bounded proof progress even when a BTCC obligation also holds
        // the engine. Its replay path consumes the same verified frontier.
        AdvancePaymentAuditPresealReplayValidation(*active_tip, durable);
        {
            LOCK(m_btcc_preseal_mutex);
            if (!m_btcc_preseal_state.IsEmpty()) return;
        }

        bool needs_full_proof{false};
        const auto inspect = [&](const auto& candidate)
            ...
            needs_full_proof |= !IsPaymentAuditPrefixAuthenticated(*terminal);
        ...
        if (needs_full_proof) {
            // Checking only the marker terminal could skip a later receipt
            // in the active tail. Authenticate the exact selected endpoint.
            if (!IsPaymentAuditReplayAuthenticated(*active_tip)) return;
            replay_source = GetPaymentAuditReplaySourceToken();
            if (replay_source.IsNull()) return;
        }
    ...
    if (!replay_source.IsNull()) {
        revalidate = [this, replay_source, replay_through, replay_through_hash]()
            EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
            const CBlockIndex* target{m_chainman.ActiveChain()[replay_through]};
            return target != nullptr && target->GetBlockHash() == replay_through_hash &&
                   replay_source == GetPaymentAuditReplaySourceToken() &&
                   IsPaymentAuditReplayAuthenticated(*target);
        };
    }
...
void CChainLocksHandler::MaybeReplayBTCCPreseal()
{
    ...
    uint256 payment_replay_source;
    {
        ...
        const auto cap_at_unauthenticated_payment =
            [&](const auto& payment_marker)
                ...
                int32_t payment_limit{earliest->nHeight - 1};
                if (IsPaymentAuditReplayValidationCurrent()) {
                    const int32_t verified_through{
                        m_payment_audit_replay_validation.frontier.ValidatedThroughHeight()};
                    if (verified_through >= earliest->nHeight) {
                        payment_limit = verified_through;
                        payment_replay_source = GetPaymentAuditReplaySourceToken();
                    }
                }
                replay_through = std::min(replay_through, payment_limit);
    ...
    std::string error;
    std::function<bool()> revalidate;
    if (!payment_replay_source.IsNull()) {
        revalidate = [this, payment_replay_source, replay_through, replay_through_hash]()
            EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
            const CBlockIndex* target{m_chainman.ActiveChain()[replay_through]};
            return target != nullptr && target->GetBlockHash() == replay_through_hash &&
                   payment_replay_source == GetPaymentAuditReplaySourceToken() &&
...
void CChainLocksHandler::RequestNeededBTCCCertificate()
{
    ...
    const auto now{GetTime<std::chrono::microseconds>()};
    {
        ...
        if (m_needed_btcc_certificate &&
            m_needed_btcc_certificate->source ==
                NeededBTCCCertificateSource::PAYMENT_AUDIT_SEAL) {
            if (m_needed_btcc_certificate->last_request.count() != 0 &&
            ...
            payment_dependency = true;
        }
    }
    {
        LOCK(m_pending_btcc_receipt_mutex);
        if (!payment_dependency && m_pending_btcc_receipt &&
            (!pending_verified ||
             pending_verified->logical_id !=
                 m_pending_btcc_receipt->logical_id)) {
    ...
    if (payment_dependency) {
        const auto capability{GetPaymentAuditSealFetchCapability(*logical_id)};
        if (!capability) return;
        if (m_store && m_store->GetVerifiedRosterAuthorizationBase(
                capability->dependency.RequestedIdentity())) {
            CompletePaymentAuditSealFetch(*capability);
            return;
        }
        CChainLockSig retained;
        if (GetChainLockByHash(*logical_id, retained)) {
            // Historical B is a byte source only. Never pass this request to
            // retained-finality promotion, even if ownership changes here.
            TRY_LOCK(m_chainlock_admission_mutex, admission_lock);
            if (!admission_lock ||
                !IsPaymentAuditSealFetchCapabilityCurrent(*capability)) {
                return;
            }
            BlockValidationState state;
            (void)ProcessPaymentAuditSealCertificate(
                /*from=*/-1, retained, state, /*peer_fault=*/nullptr);
            return;
        }
    }
    ...
    if (!payment_dependency && TryPromoteRetainedChainLock(*logical_id) !=
...
void CChainLocksHandler::RequestNeededPaymentAuditCertificate()
{
    ...
    if (!m_chainman.IsPQParticipationAllowed()) return;
    {
        ...
        std::optional<PendingPaymentAuditSealDependency> stalled_seal;
        {
            LOCK(m_pending_payment_audit_receipt_mutex);
            stalled_seal = m_pending_payment_audit_seal;
        }
        if (stalled_seal && SelectPaymentAuditHistoricalReplayBoundary(stalled_seal->owner)) {
            LOCK(m_pending_payment_audit_receipt_mutex);
            LOCK(m_needed_btcc_certificate_mutex);
            if (m_pending_payment_audit_seal == stalled_seal &&
                m_pending_payment_audit_receipt == stalled_seal->owner &&
                m_needed_btcc_certificate &&
                m_needed_btcc_certificate->source == NeededBTCCCertificateSource::PAYMENT_AUDIT_SEAL &&
                m_needed_btcc_certificate->source_token == stalled_seal->source_token &&
                m_needed_btcc_certificate->logical_id == stalled_seal->RequestedIdentity().logical_id) {
                // This terminal was too recent when its ordinary dependency
                // was requested. Its independently selected E now permits
                // replay verification from the exact audit itself.
                (void)EraseNeededBTCCCertificate(m_needed_btcc_certificate,
                    NeededBTCCCertificateSource::PAYMENT_AUDIT_SEAL, stalled_seal->source_token);
                m_pending_payment_audit_seal.reset();
                m_pending_payment_audit_last_request = std::chrono::microseconds{0};
            }
        }
    }
    std::optional<uint256> seal_logical_id;
    {
        LOCK(m_pending_payment_audit_receipt_mutex);
        if (m_pending_payment_audit_seal) {
            seal_logical_id =
    ...
    if (seal_logical_id) {
        const auto capability{
            GetPaymentAuditSealFetchCapability(*seal_logical_id)};
        if (capability && m_store &&
            m_store->GetVerifiedRosterAuthorizationBase(
                capability->dependency.RequestedIdentity())) {
            CompletePaymentAuditSealFetch(*capability);
        }
        // GETCLSIG is owned by the single needed-certificate scheduler lane.
        // Never re-request the large audit until its exact seal is available.
        return;
    }
    ...
    HistoricalAdmission admission) const
{
    ...
    auto proof_config{*m_config};
    const auto imported{GetPoWHistoricalSyncAuthorization()};
    const bool covered{imported &&
        candidate.nHeight > imported->boundary.coverage_height &&
        candidate.GetAncestor(imported->boundary.coverage_height)->GetBlockHash() ==
            imported->boundary.coverage_hash};
    if (covered) {
        proof_config.btcc_receipt_assumption_anchor = {
            imported->boundary.coverage_height, imported->boundary.coverage_hash,
            imported->boundary.receipt_state};
    }
    ...
    const uint256 branch_token{
        CatchupValidationDomainToken(m_chainman, proof_config,
                                     marker_token)};
    const uint256 context_token{
        CatchupHistoricalContextToken(candidate, m_chainman, proof_config,
                                      marker_token)};
    return m_catchup_proof_cache.GetOrCompute(
        ...
        [&]() EXCLUSIVE_LOCKS_REQUIRED(cs_main)
            ...
            // The immutable imported endpoint already covers the old marker
            // range. Only the fully validated suffix above it is new work.
            if (covered) return {*indexed, true};
    ...
    const CBlockIndex** candidate_out) const
{
    ...
    int32_t validation_floor{CandidateFullValidationFloor(
        request, m_config->activation_predecessor_height)};
    const auto imported{catchup ? GetPoWHistoricalSyncAuthorization() : nullptr};
    const bool historical_prefix{imported &&
        candidate->nHeight > imported->boundary.coverage_height &&
        candidate->GetAncestor(imported->boundary.coverage_height)->GetBlockHash() ==
            imported->boundary.coverage_hash};
    if (historical_prefix) {
        validation_floor = std::max(validation_floor, imported->boundary.coverage_height);
    }
    ...
    const bool catchup_historical_receipt_range{
        catchup && !historical_prefix && !exact_local_target && validation_floor >= 0 &&
        ClassifyHistoricalReceiptIndexRangeCached(
            *candidate, validation_floor) ==
            PaymentAuditContextStatus::READY};
    ...
    if (candidate_out != nullptr) *candidate_out = candidate;
    uint256 admission_token{(marker_authorized_historical ||
                            (current_round_candidate && !active_candidate))
                               ? marker_token : uint256{}};
    if (historical_prefix) {
        CHashWriter writer{SER_GETHASH, 0};
        writer << admission_token << pq::GetHistoricalSyncBoundaryHash(
            m_genesis_hash, *m_config, imported->boundary)
               << imported->provenance_revision;
        admission_token = writer.GetHash();
    }
    return pq::ChainLockCandidateContext{
        true,
        validated,
...
{
    LOCK(cs_main);
    return FlushBTCCIndexStateForDurableAcceptanceLocked(chainlock);
}

bool CChainLocksHandler::FlushBTCCIndexStateForDurableAcceptanceLocked(
    const pq::FinalChainLock& chainlock) const
{
    AssertLockHeld(cs_main);
...
CChainLocksHandler::ResolveObjectiveRosterAuthorizationContext(
    const CBlockIndex& candidate,
    const pq::VerifiedRosterAuthorizationBaseView* exact_base,
    std::optional<pq::RosterAuthorizationBaseIdentity>* selected_identity) const
{
    AssertLockHeld(cs_main);
    if (selected_identity != nullptr) selected_identity->reset();
    ...
    // This identity only authorizes fetching the exact receipt-selected
    // certificate. It never supplies the missing ordinary authorization edge.
    if (selected_identity != nullptr) {
        *selected_identity = pq::RosterAuthorizationBaseIdentity{
            receipted_target_height, receipted_target->GetBlockHash(),
            receipt->chainlock_logical_id};
    }
    auto base{exact_base != nullptr
        ? std::optional<pq::VerifiedRosterAuthorizationBaseView>{*exact_base}
        : m_store->GetVerifiedRosterAuthorizationBaseByLogicalId(
              receipt->chainlock_logical_id)};
    if (!base && exact_base == nullptr) {
        const auto imported{GetPoWHistoricalSyncAuthorization()};
        if (imported && imported->boundary.receipt == *receipt &&
            imported->boundary.carrier_hash == carrier->GetBlockHash() &&
            candidate.nHeight > imported->boundary.coverage_height &&
    ...
    if (*mode == pq::ObjectiveRosterAuthorizationMode::RECOVER) {
        if (!m_quorum_build_config) return std::nullopt;
        const auto source_mode{pq::GetRecoveryRefreshMode(
            m_config->chainlock_schedule, m_config->btcc_schedule,
            m_quorum_build_config->recovery_refresh, candidate.nHeight,
            receipted_target_height)};
        if (!source_mode) return std::nullopt;
        if (*source_mode == pq::RecoveryRefreshMode::POW_REFRESHED_SOURCE) {
            pq::FrozenQuorumRosterCachePtr cache;
            {
                LOCK(m_lookup_mutex);
                cache = m_quorum_roster_cache;
            }
            const auto universe{cache ? cache->BuildPoWRefreshUniverse(
                *target_epoch / pq::ACTIVE_QUORUMS, candidate) : nullptr};
            if (!universe) return std::nullopt;
            source = universe->Source();
            result.refresh_authority =
                std::shared_ptr<const pq::VerifiedPoWRefreshAuthority>{
                    new pq::VerifiedPoWRefreshAuthority{
                        m_genesis_hash, candidate.nHeight,
                        candidate.GetBlockHash(), source}};
        }
    }
    ...
    const ObjectiveRosterAuthorizationContext* objective) const
{
    ...
    if (transition == pq::RosterAuthorizationTransitionKind::RECOVER) {
        ...
        authorization.m_pow_refresh = objective->refresh_authority;
        return authorization;
    }
...
bool CChainLocksHandler::IsHistoricalAuthorizationBaseAheadOfDurableWinner(
    const pq::FinalChainLockRecordMetadata* current,
    const pq::FinalChainLockRecordMetadata& historical_base) noexcept
{
    return !current || current->statement.height < historical_base.statement.height;
}
        ...
        btcc_cursor_reconciliation) const
{
    ...
    if (candidate_admission == pq::ChainLockCandidateAdmission::CATCHUP && exact_prior &&
        ...
        bool exact_historical_base{false};
        {
            LOCK(cs_main);
            const auto imported{GetPoWHistoricalSyncAuthorization()};
            exact_historical_base = imported &&
                exact_prior->metadata == imported->base.metadata &&
        ...
        if (exact_historical_base) {
            // The imported receipt authorizes this one missing historical base,
            // not the new state edge. Validate that edge under ordinary rules.
            const auto mask{pq::ValidateRosterAuthorizationState(
    ...
    if (statement.roster_transition ==
        ...
        auto expected_source{current_bundle.recovery_authority_source};
        std::shared_ptr<const pq::VerifiedPoWRefreshAuthority> refresh_authority;
        if (exact_authorization.HasPoWRefreshAuthority(m_genesis_hash, statement)) {
            expected_source = candidate_bundle.recovery_authority_source;
            refresh_authority = exact_authorization.m_pow_refresh;
        }
        if (!target_epoch || !canonical ||
        ...
        projected.m_pow_refresh = std::move(refresh_authority);
...
bool CChainLocksHandler::AdvanceLiveSigningValidationFrontier(
    ...
    std::size_t block_budget,
    const pq::HistoricalSyncBoundary* historical_coverage)
{
    AssertLockHeld(cs_main);
    ...
    if (historical_coverage != nullptr) {
        const auto& coverage{*historical_coverage};
        const CBlockIndex* endpoint{active_chain[coverage.coverage_height]};
        const CBlockIndex* carrier{active_chain[coverage.carrier_height]};
        const CBlockIndex* base{active_chain[coverage.receipt.chainlock_target_height]};
        const auto historical_provenance = [](const CBlockIndex* index)
            EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
            return index && !(index->nStatus & BLOCK_FAILED_MASK) &&
                !index->IsAssumedValid() && index->IsValid(BLOCK_VALID_SCRIPTS) &&
                HasFullReceiptIndexProvenance(*index);
        };
        const bool prior_matches{coverage.durable_prior.IsNull()
            ? durable_predecessor.height == config.activation_predecessor_height &&
                  durable_predecessor.btcc_cursor.IsNull()
            : coverage.durable_prior.height == durable_predecessor.height &&
                  coverage.durable_prior.block_hash == durable_predecessor.block_hash};
        const auto receipt_state{endpoint
            ? IndexedBTCCReceiptState(*endpoint) : std::nullopt};
        const auto payment_state{endpoint
            ? IndexedPaymentAuditReceiptState(*endpoint) : std::nullopt};
        const auto carrier_prior{carrier && carrier->pprev
            ? IndexedBTCCReceiptState(*carrier->pprev) : std::nullopt};
        const auto carrier_state{carrier
            ? IndexedBTCCReceiptState(*carrier) : std::nullopt};
        const auto receipt{carrier && carrier_prior && carrier_state
            ? pq::ReconstructBTCCReceipt(
                  genesis_hash, config.chainlock_schedule, config.btcc_schedule,
                  config.activation_predecessor_height, *carrier,
                  *carrier_prior, *carrier_state, carrier->pqBTCCReceiptLogicalId)
            : std::nullopt};
        if (!coverage.IsStructurallyValid() || !prior_matches ||
            coverage.coverage_height <= durable_predecessor.height ||
            coverage.coverage_height >= target.nHeight ||
            coverage.receipt.chainlock_target_height <= config.activation_predecessor_height ||
            (!coverage.durable_prior.IsNull() && !historical_provenance(active_floor)) ||
            !historical_provenance(base) || !historical_provenance(carrier) ||
            !historical_provenance(carrier ? carrier->pprev : nullptr) ||
            !historical_provenance(endpoint) ||
            base->GetBlockHash() != coverage.receipt.chainlock_target_hash ||
            (coverage.receipt.chainlock_target_height == durable_predecessor.height &&
             (coverage.durable_prior.logical_id != coverage.receipt.chainlock_logical_id ||
              durable_predecessor.btcc_cursor != coverage.receipt.accepted_cursor)) ||
            endpoint == nullptr || endpoint->GetBlockHash() != coverage.coverage_hash ||
            carrier == nullptr || carrier->GetBlockHash() != coverage.carrier_hash ||
            !receipt_state || *receipt_state != coverage.receipt_state ||
            !payment_state || *payment_state != coverage.payment_audit_state ||
            endpoint->pqPaymentProbationStateHash != coverage.probation_state_hash ||
            !receipt || *receipt != coverage.receipt) {
            frontier = {};
            return false;
        }
        historical_coverage_token = pq::GetHistoricalSyncBoundaryHash(
            genesis_hash, config, coverage);
        if (historical_coverage_token.IsNull()) {
            frontier = {};
            return false;
        }
    }
    ...
    const auto reset_to_floor = [&] {
        ...
        frontier.provenance_revocation_revision =
            provenance_revocation_revision;
        frontier.historical_coverage_token = historical_coverage_token;
    ...
    for (int32_t height{frontier.validated_through_height + 1};
        ...
        // Established PoW history supplies deferred certificate/governance
        // authorization only through this exact endpoint. It neither records
        // individual verification nor relaxes the remaining live suffix.
        const bool historically_covered{historical_coverage != nullptr &&
            height <= historical_coverage->coverage_height};
        if (index == nullptr || index->nHeight != height ||
            (index->nStatus & BLOCK_FAILED_MASK) ||
            index->IsAssumedValid() ||
            !index->IsValid(BLOCK_VALID_SCRIPTS) ||
            !HasFullReceiptIndexProvenance(*index) ||
            (!historically_covered && CSuperblock::IsValidBlockHeight(height) &&
             !(index->nStatus & BLOCK_GOVERNANCE_VALIDATED))) {
            return false;
        }
...
bool CChainLocksHandler::ShouldExtendPoWHistoricalCoverageForGovernance(
    const pq::HistoricalSyncBoundary& established,
    const pq::HistoricalSyncBoundary& selected,
    ...
    const pq::ChainLockFinalityStoreConfig& config)
{
    AssertLockHeld(cs_main);
    if (!frontier.initialized ||
        frontier.provenance_revocation_revision != provenance_revision ||
        frontier.historical_coverage_token.IsNull() ||
        frontier.historical_coverage_token !=
            pq::GetHistoricalSyncBoundaryHash(genesis_hash, config, established) ||
        !selected.IsStructurallyValid() ||
        established.receipt != selected.receipt ||
        established.carrier_hash != selected.carrier_hash ||
        established.durable_prior != selected.durable_prior ||
        selected.coverage_height <= established.coverage_height ||
        frontier.validated_through_height < established.coverage_height ||
        frontier.validated_through_height >= selected.coverage_height) {
        return false;
    }
    const CBlockIndex* previous_end{active_chain[established.coverage_height]};
    const CBlockIndex* selected_end{active_chain[selected.coverage_height]};
    const CBlockIndex* validated{active_chain[frontier.validated_through_height]};
    const CBlockIndex* blocked{active_chain[frontier.validated_through_height + 1]};
    // A scan budget or an alternate witness is not a reason to move E. The
    // next block must instead prove a specific deferred historical obligation.
    return previous_end && selected_end && validated && blocked &&
        previous_end->GetBlockHash() == established.coverage_hash &&
        selected_end->GetBlockHash() == selected.coverage_hash &&
        validated->GetBlockHash() == frontier.validated_through_hash &&
        CSuperblock::IsValidBlockHeight(blocked->nHeight) &&
        !(blocked->nStatus & (BLOCK_FAILED_MASK | BLOCK_GOVERNANCE_VALIDATED)) &&
        !blocked->IsAssumedValid() && blocked->IsValid(BLOCK_VALID_SCRIPTS) &&
        HasFullReceiptIndexProvenance(*blocked);
}
    ...
    uint64_t admission_generation)
{
    ...
    const auto payment_audit_checkpoint{
        m_payment_audit_store->GetPruneCheckpoint()};
    if (!m_payment_audit_store->IsHealthy()) return std::nullopt;
    const auto persisted_recovery{
        m_persistence->LoadRosterRecoveryPrecommit()};
    auto staged_recovery{persisted_recovery};
    ...
    pq::RecoveryRosterAuthoritySource recovery_authority_source;
    {
        ...
        const auto imported{GetPoWHistoricalSyncAuthorization()};
        const auto historical{imported &&
                imported->boundary.coverage_height > durable_predecessor.height
            ? imported : nullptr};
        if (historical) {
            // The imported authority supersedes an unfinished INITIALIZE
            // attempt, not its durable journal or the actual finality floor.
            staged_recovery.reset();
            source.historical_coverage_token = pq::GetHistoricalSyncBoundaryHash(
                m_genesis_hash, *m_config, historical->boundary);
            if (source.historical_coverage_token.IsNull()) return std::nullopt;
        }
        ...
        if (!AdvanceLiveSigningValidationFrontier(
                ...
                *indexed_target, durable_predecessor, *m_config,
                ...
                HistoricalIndexValidationCache::BLOCK_BUDGET,
                historical ? &historical->boundary : nullptr)) {
            return std::nullopt;
        }
        ClearNeededBTCCCertificate(
            NeededBTCCCertificateSource::LIVE_FRONTIER,
        ...
        source.provenance_revocation_revision =
            provenance_revocation_revision;
        const uint256 recovery_token{persisted_recovery
            ? RosterRecoveryPrecommitToken(
                  m_genesis_hash, *persisted_recovery)
            : uint256{}};
        if (persisted_recovery && recovery_token.IsNull()) {
            return std::nullopt;
        }
        source.mutable_signing_context_token =
            MutableSigningContextToken(
        ...
        source.outage_recovery = objective_authorization.has_value() && reset_path;
    }
    ...
    if (reset_path) {
        ...
        const bool outage_recovery{contexts.source.outage_recovery};
        if (outage_recovery) {
            ...
            authorization.m_pow_refresh = objective_authorization->refresh_authority;
    ...
    const CurrentSigningSource& source) const
{
    ...
    const bool uses_historical_coverage{!source.historical_coverage_token.IsNull()};
    if (finality.state_revision != source.finality_store_revision ||
        ...
        (source.outage_recovery &&
         (source.staged_recovery ||
          (!source.has_durable_chainlock && !uses_historical_coverage))) ||
        mutable_signing_context_token !=
            source.mutable_signing_context_token ||
        source.staged_recovery !=
            (recovery_precommit.has_value() && !uses_historical_coverage) ||
        payment_audit_preseal_token !=
            source.payment_audit_preseal_token ||
        !IsQuorumRosterSourceGenerationCurrent(
    ...
    const auto branch_is_current = [&]() {
        LOCK(cs_main);
        if (uses_historical_coverage) {
            const auto historical{GetPoWHistoricalSyncAuthorization()};
            if (!historical ||
                historical->boundary.coverage_height <=
                    source.durable_predecessor.height ||
                historical->boundary.coverage_height >= source.window.target_height ||
                pq::GetHistoricalSyncBoundaryHash(
                    m_genesis_hash, *m_config, historical->boundary) !=
                    source.historical_coverage_token) {
                return false;
            }
        }
        const CBlockIndex* tip{m_chainman.ActiveTip()};
    ...
    return IsShareAdmissionGenerationCurrent(source.admission_generation) &&
           ...
           IsQuorumRosterSourceGenerationCurrent(
               source.roster_source_generation) &&
           (!uses_historical_coverage || branch_is_current());
}
    ...
    uint8_t row_index) const
{
    ...
    if (!roster_set || roster_set->Rosters().back().descriptor.epoch != epoch ||
        response_statement->roster_beacons.active.seeds.back().IsRecovery() ||
...
std::optional<CChainLocksHandler::PaymentAuditHistoricalReplayBoundary>
CChainLocksHandler::GetPaymentAuditHistoricalReplayBoundaryView(
    const PendingPaymentAuditReceiptDependency& owner,
    std::optional<int32_t> coverage_height) const
{
    AssertLockHeld(cs_main);
    AssertLockNotHeld(m_lookup_mutex);
    AssertLockNotHeld(m_btcc_preseal_mutex);
    // Observe marker/source state without m_store or dependency/frontier
    // queries: this bounded check also precedes external replay operations.
    if (!m_config || !m_persistence || m_persistence_failed.load() ||
        !m_chainman.IsBaseBlockSyncComplete() ||
        (m_chainman.IsSnapshotActive() && !m_chainman.IsSnapshotValidated()) ||
        owner.source != PaymentAuditReceiptDependencySource::PRESEAL_REPLAY ||
        owner.source_token.IsNull() ||
        owner.source_token != GetPaymentAuditReplaySourceToken() ||
        owner.receipt.IsNull() || !owner.receipt.IsStructurallyValid()) {
        return std::nullopt;
    }
    const CChain& chain{m_chainman.ActiveChain()};
    const CBlockIndex* active_tip{chain.Tip()};
    const CBlockIndex* selection_tip{active_tip
        ? ResolvePoWHistoricalSelectionTip(
              *active_tip, m_config->chainlock_schedule, coverage_height)
        : nullptr};
    const auto target{selection_tip ? pq::LatestEligibleChainLockTargetHeight(
        m_config->chainlock_schedule, selection_tip->nHeight) : std::nullopt};
    if (!target || *target <
            static_cast<int32_t>(m_config->chainlock_schedule.sign_lag)) {
        return std::nullopt;
    }
    const CBlockIndex* coverage{selection_tip->GetAncestor(
        *target - m_config->chainlock_schedule.sign_lag)};
    const auto usable = [](const CBlockIndex* index)
        EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
        return index && !index->IsAssumedValid() &&
               index->IsValid(BLOCK_VALID_SCRIPTS) &&
               !(index->nStatus & (BLOCK_FAILED_MASK | BLOCK_CONFLICT_CHAINLOCK)) &&
               HasFullReceiptIndexProvenance(*index);
    };
    const CBlockIndex* carrier{chain[owner.receipt.carrier_height]};
    if (!usable(coverage) || !usable(carrier) || !usable(carrier->pprev) ||
        carrier->GetBlockHash() != owner.carrier_hash ||
        carrier->pprev->GetBlockHash() != owner.carrier_parent_hash ||
        carrier->nHeight > coverage->nHeight) return std::nullopt;
    if (ClassifyPaymentAuditReceiptCarrierContext(owner.receipt, *carrier,
            pq::PaymentAuditScheduleConfig{m_config->chainlock_schedule, m_config->btcc_schedule}) !=
        PaymentAuditContextStatus::READY) return std::nullopt;
    const auto durable{m_persistence->GetFinalityState().best};
    const int32_t floor{durable ? durable->statement.height
        : m_config->activation_predecessor_height};
    const CBlockIndex* predecessor{chain[floor]};
    if (!predecessor || coverage->nHeight <= floor ||
        (durable && predecessor->GetBlockHash() != durable->statement.block_hash)) {
        return std::nullopt;
    }
    ...
    if (!markers.IsStructurallyValid()) return std::nullopt;
    const pq::PaymentAuditPresealMarker* latest{nullptr};
    for (const auto* candidate : {&markers.active, &markers.prospective}) {
        if (!*candidate) continue;
        const auto& marker{**candidate};
        const CBlockIndex* terminal{chain[marker.terminal_carrier_height]};
        if (!terminal || terminal->GetBlockHash() != marker.terminal_carrier_hash) continue;
        const CBlockIndex* earliest{chain[marker.earliest_carrier_height]};
        if (!usable(earliest) || !usable(earliest->pprev) ||
            !usable(terminal) || !usable(terminal->pprev) ||
            earliest->GetBlockHash() != marker.earliest_carrier_hash ||
            IndexedPaymentAuditReceiptState(*earliest->pprev) !=
                marker.predecessor_receipt_state ||
            earliest->pprev->pqPaymentProbationStateHash !=
                marker.predecessor_probation_state_hash) return std::nullopt;
        if (!latest || marker.terminal_carrier_height > latest->terminal_carrier_height) {
            latest = &marker;
        }
    }
    if (!latest || latest->terminal_carrier_hash != owner.carrier_hash ||
        latest->terminal_receipt != owner.receipt) return std::nullopt;
    const auto previous{IndexedPaymentAuditReceiptState(*carrier->pprev)};
    const auto applied{previous ? pq::ApplyPaymentAuditReceipt(
        m_genesis_hash, *previous, owner.receipt) : std::nullopt};
    const auto btcc{IndexedBTCCReceiptState(*coverage)};
    const auto audit{IndexedPaymentAuditReceiptState(*coverage)};
    if (!applied || applied != IndexedPaymentAuditReceiptState(*carrier) ||
        owner.receipt.next_probation_state_hash != carrier->pqPaymentProbationStateHash ||
        !btcc || !audit || coverage->pqPaymentProbationStateHash.IsNull()) {
        return std::nullopt;
    }
    return PaymentAuditHistoricalReplayBoundary{owner, coverage->nHeight,
        coverage->GetBlockHash(), durable, *btcc, *audit,
        coverage->pqPaymentProbationStateHash};
}

std::optional<CChainLocksHandler::PaymentAuditHistoricalReplayBoundary>
CChainLocksHandler::SelectPaymentAuditHistoricalReplayBoundary(
    const PendingPaymentAuditReceiptDependency& owner,
    std::optional<int32_t> coverage_height) const
{
    AssertLockHeld(cs_main);
    const auto selected{GetPaymentAuditHistoricalReplayBoundaryView(owner, coverage_height)};
    if (!selected || !m_store) return std::nullopt;
    const auto accepted{m_store->GetBestRecord()};
    if (accepted.has_value() != selected->durable_prior.has_value() ||
        (accepted && accepted->metadata != *selected->durable_prior)) return std::nullopt;
    const CBlockIndex* coverage{m_chainman.ActiveChain()[selected->coverage_height]};
    const int32_t floor{selected->durable_prior
        ? selected->durable_prior->statement.height : m_config->activation_predecessor_height};
    if (ClassifyHistoricalReceiptIndexRangeCached(*coverage, floor + 1) !=
            PaymentAuditContextStatus::READY) return std::nullopt;
    const CBlockIndex* carrier{m_chainman.ActiveChain()[owner.receipt.carrier_height]};
    CBlock block;
    pq::PaymentAuditReceipt decoded;
    if (!m_chainman.m_blockman.ReadBlockFromDisk(block, *carrier) ||
        block.GetHash() != owner.carrier_hash ||
        !ExtractPaymentAuditReceipt(block, decoded) || decoded != owner.receipt) {
        return std::nullopt;
    }
    return selected;
}
...
pq::PreparedChainLockContextPtr
CChainLocksHandler::BuildPaymentAuditHistoricalReplaySealContext(
    const PaymentAuditHistoricalReplayBoundary& boundary,
    const pq::PaymentAuditStatement& statement) const
{
    AssertLockHeld(cs_main);
    const auto selected{SelectPaymentAuditHistoricalReplayBoundary(
        boundary.owner, boundary.coverage_height)};
    const auto& receipt{boundary.owner.receipt};
    const auto& seal_statement{statement.seal_statement};
    const CBlockIndex* seal{m_chainman.ActiveChain()[receipt.seal_height]};
    if (!selected || *selected != boundary || !statement.IsStructurallyValid() ||
        pq::GetPaymentAuditLogicalId(m_genesis_hash, statement) != receipt.audit_logical_id ||
        seal == nullptr || seal->GetBlockHash() != receipt.seal_block_hash ||
        seal_statement.height != receipt.seal_height ||
        seal_statement.block_hash != receipt.seal_block_hash ||
        !IsPaymentAuditReplayFloorStatementValid(seal_statement, *seal)) return nullptr;
    const auto cache{GetQuorumRosterCache()};
    const auto rosters{cache ? cache->GetVerifiedActiveNoPublish(
        seal->nHeight, *seal, seal_statement.roster_beacons.active) : nullptr};
    if (!rosters) return nullptr;
    CHashWriter writer{SER_GETHASH, 0};
    writer << std::string{"SYS_PQ_PAYMENT_AUDIT_HISTORICAL_REPLAY_V1"}
           << m_genesis_hash << receipt << boundary.owner.carrier_hash
           << boundary.owner.carrier_parent_hash << boundary.owner.source_token
           << boundary.coverage_height << boundary.coverage_hash
           << boundary.receipt_state << boundary.payment_audit_state
           << boundary.probation_state_hash << boundary.durable_prior.has_value();
    if (boundary.durable_prior) {
        writer << boundary.durable_prior->statement << boundary.durable_prior->logical_id
               << boundary.durable_prior->witness_id;
    }
    const pq::VerifiedPoWHistoricalBoundary proof{
        m_genesis_hash, m_config->chainlock_schedule,
        pq::GetLogicalChainLockId(m_genesis_hash, seal_statement), writer.GetHash()};
    return pq::PreparedChainLockContext::CreateFromPoWHistory(
        m_config->chainlock_schedule, seal_statement, rosters, proof);
}

VerifiedPaymentAuditReceiptTransitionPtr
CChainLocksHandler::GetPaymentAuditHistoricalReplayTransition(
    const pq::PaymentAuditReceipt& receipt, const CBlockIndex& carrier) const
{
    AssertLockHeld(cs_main);
    const auto& proof{m_payment_audit_historical_replay_proof};
    if (!proof || !proof->transition || !deterministicMNManager) return nullptr;
    const auto& verified{*proof->transition};
    const auto& boundary{proof->boundary};
    const auto current{GetPaymentAuditHistoricalReplayBoundaryView(
        boundary.owner, boundary.coverage_height)};
    const CBlockIndex* seal{m_chainman.ActiveChain()[receipt.seal_height]};
    if (!current || *current != boundary || receipt != boundary.owner.receipt ||
        receipt != verified.m_receipt || carrier.GetBlockHash() != boundary.owner.carrier_hash ||
        carrier.nHeight != receipt.carrier_height || !carrier.pprev ||
        carrier.pprev->GetBlockHash() != verified.m_carrier_parent_hash ||
        carrier.pprev->nHeight != verified.m_carrier_parent_height ||
        carrier.pprev->pqPaymentProbationStateHash != verified.m_parent_probation_state_hash ||
        verified.m_statement.commitment.previous_probation_state_hash !=
            verified.m_parent_probation_state_hash ||
        verified.m_archive_revision != 0 ||
        !IsQuorumRosterSourceGenerationCurrent(verified.m_roster_source_generation) ||
        deterministicMNManager->PaymentProbationStateViewGeneration() !=
            verified.m_probation_state_view_generation ||
        !seal || seal->IsAssumedValid() || !seal->IsValid(BLOCK_VALID_SCRIPTS) ||
        (seal->nStatus & (BLOCK_FAILED_MASK | BLOCK_CONFLICT_CHAINLOCK)) ||
        !HasFullReceiptIndexProvenance(*seal) ||
        !IsPaymentAuditReplayFloorStatementValid(verified.m_statement.seal_statement, *seal)) {
        return nullptr;
    }
    const pq::PQPaymentAuditReceiptIdentity identity{
        receipt.epoch, receipt.carrier_height, receipt.result_hash};
    const auto& transition{verified.m_transition};
    if (transition.ProvenanceGeneration() != verified.m_probation_state_view_generation ||
        transition.PreviousStateHash() != verified.m_parent_probation_state_hash ||
        transition.AppliedReceipt() != identity ||
        transition.Result().StateHash() != receipt.next_probation_state_hash) return nullptr;
...
{
    AssertLockNotHeld(cs_main);
    const auto& owner{historical.dependency};
    if (owner.source != PaymentAuditReceiptDependencySource::PRESEAL_REPLAY || !m_config) {
        return std::nullopt;
    }
    const auto& receipt{owner.receipt};
    std::optional<PaymentAuditHistoricalReplayBoundary> boundary;
    pq::PreparedChainLockContextPtr seal_context;
    ScopedFinalitySnapshotVerificationRetention retention{deterministicMNManager.get()};
    {
        LOCK(cs_main);
        // PoW-history selection starts only after base synchronization.
        // Ordinary certificate verification remains available before then.
        if (!m_chainman.IsBaseBlockSyncComplete() ||
            (m_chainman.IsSnapshotActive() && !m_chainman.IsSnapshotValidated())) {
            return std::nullopt;
        }
        const CBlockIndex* tip{m_chainman.ActiveTip()};
        const auto target{tip ? pq::LatestEligibleChainLockTargetHeight(
            m_config->chainlock_schedule, tip->nHeight) : std::nullopt};
        if (!target || int64_t{receipt.carrier_height} >
                int64_t{*target} - m_config->chainlock_schedule.sign_lag) return std::nullopt;
        const auto current{ResolvePendingPaymentAuditContext(receipt.audit_witness_id)};
        if (!current || *current != historical) return false;
        boundary = SelectPaymentAuditHistoricalReplayBoundary(owner);
        if (!boundary) return false;
        seal_context = BuildPaymentAuditHistoricalReplaySealContext(*boundary, audit.statement);
    }
    const auto classification{pq::ClassifyPaymentAuditReports(audit)};
    const auto* subject_beacon{pq::FindRosterBeaconSeed(
        audit.statement.seal_statement.roster_beacons.active, receipt.epoch)};
    if (!seal_context || !audit.IsStructurallyValid() || !classification ||
        audit.GetLogicalId(m_genesis_hash) != receipt.audit_logical_id ||
        audit.GetWitnessId(m_genesis_hash) != receipt.audit_witness_id ||
        audit.statement.commitment.seed.epoch != receipt.epoch ||
        pq::GetPaymentAuditCommitmentHash(m_genesis_hash, audit.statement.commitment) != receipt.commitment_hash ||
        pq::GetPaymentAuditResultHash(m_genesis_hash, audit, *classification) != receipt.result_hash ||
        classification->online_members != receipt.online_members ||
        !subject_beacon || *subject_beacon != receipt.subject_roster_beacon) return false;
    pq::RosterAuthorizationVerificationContext authorization;
    uint64_t roster_generation{0};
    int32_t reconstruction_floor{-1};
    const auto rosters{BuildPaymentAuditVerificationRosters(audit.statement,
        nullptr, &authorization, false, nullptr, &historical, &roster_generation,
        &reconstruction_floor, true, nullptr, seal_context)};
    const pq::PaymentAuditScheduleConfig schedule{m_config->chainlock_schedule, m_config->btcc_schedule};
    auto prepared{rosters ? pq::PrepareFinalPaymentAuditVerification(
        schedule, audit, rosters, authorization) : std::nullopt};
    if (!prepared) return false;
    {
        LOCK(m_verification_mutex);
        if (!m_verifier.VerifyChecks(std::move(prepared->checks))) return false;
    }
    return m_chainman.ActiveChainstate().RunWithStableActiveChain([&]() EXCLUSIVE_LOCKS_REQUIRED(!cs_main) {
        pq::PreparedChainLockContextPtr current_seal;
        {
            LOCK(cs_main);
            const auto current{ResolvePendingPaymentAuditContext(receipt.audit_witness_id)};
            if (!current || *current != historical) return false;
            current_seal = BuildPaymentAuditHistoricalReplaySealContext(*boundary, audit.statement);
        }
        if (!current_seal) return false;
        pq::FrozenQuorumRoster subject;
        pq::RosterAuthorizationVerificationContext current_authorization;
        uint64_t current_generation{0};
        int32_t current_floor{-1};
        const auto current_rosters{BuildPaymentAuditVerificationRosters(audit.statement,
            &subject, &current_authorization, false, nullptr, &historical,
            &current_generation, &current_floor, false, nullptr, current_seal)};
        if (!current_rosters || current_generation != roster_generation ||
            current_floor != reconstruction_floor ||
            current_seal->AuthorizationMask() != seal_context->AuthorizationMask()) return false;
        for (std::size_t slot{0}; slot < rosters->Rosters().size(); ++slot) {
            if (!SameFrozenQuorumRoster(rosters->Rosters()[slot], current_rosters->Rosters()[slot])) {
                return false;
            }
        }
        LOCK(cs_main);
        const auto current{ResolvePendingPaymentAuditContext(receipt.audit_witness_id)};
        const auto selected{SelectPaymentAuditHistoricalReplayBoundary(owner, boundary->coverage_height)};
        if (!current || *current != historical || !selected || *selected != *boundary ||
            !IsQuorumRosterSourceGenerationCurrent(roster_generation)) return false;
        const CBlockIndex* carrier{m_chainman.ActiveChain()[receipt.carrier_height]};
        ...
        VerifiedPaymentAuditReceiptTransitionPtr verified{new VerifiedPaymentAuditReceiptTransition{
            ...
            carrier->pprev->pqPaymentProbationStateHash, 0, roster_generation,
            probation_generation, reconstruction_floor, seal_context->AuthorizationMask(),
            std::move(*transition)}};
        // The historical trust substitute belongs only to this RAM proof.
        // It never enters the ordinary audit archive or finality store.
        m_payment_audit_historical_replay_proof =
            std::make_shared<const PaymentAuditHistoricalReplayProof>(
                PaymentAuditHistoricalReplayProof{*boundary, std::move(verified)});
        if (!GetPaymentAuditHistoricalReplayTransition(receipt, *carrier)) {
            m_payment_audit_historical_replay_proof.reset();
            return false;
        }
        return true;
    });
...
CChainLocksHandler::BuildPaymentAuditVerificationRosters(
    ...
    bool defer_historical_provenance,
    std::optional<PendingPaymentAuditSealDependency>*
        missing_seal,
    pq::PreparedChainLockContextPtr historical_seal_context) const
{
    if (status != nullptr) {
    ...
    if (missing_seal != nullptr) missing_seal->reset();
    if (!statement.IsStructurallyValid() ||
    ...
    auto seal_context{std::move(historical_seal_context)};
    if (seal_context &&
        (historical == nullptr ||
         historical->dependency.source !=
             PaymentAuditReceiptDependencySource::PRESEAL_REPLAY ||
         seal_context->GenesisHash() != m_genesis_hash ||
         seal_context->Schedule() != m_config->chainlock_schedule ||
         seal_context->Statement() != statement.seal_statement ||
         seal_context->Authorization().admission !=
             pq::RosterAuthorizationAdmission::POW_HISTORY)) {
        return nullptr;
    }
    if (!seal_context) {
        ...
        if (!accepted_seal_record) {
            if (historical == nullptr || m_store == nullptr) {
                if (status != nullptr) {
                    *status = PaymentAuditRosterBuildStatus::LOCAL_ERROR;
                }
                return nullptr;
            }
            std::optional<pq::RosterAuthorizationBaseIdentity> objective_base;
            if (statement.seal_statement.roster_transition !=
                pq::RosterAuthorizationTransitionKind::INITIALIZE) {
                (void)ResolveObjectiveRosterAuthorizationContext(
                    *seal, nullptr, &objective_base);
                if (!objective_base) {
                    if (status != nullptr) {
                        *status = PaymentAuditRosterBuildStatus::LOCAL_ERROR;
                    }
                ...
                if (*objective_base !=
                    statement.seal_statement.roster_authorization_base) {
                    return nullptr;
                }
            }
            const auto dependency{MakePendingPaymentAuditSealDependency(
                m_genesis_hash, historical->dependency,
                statement.seal_statement, objective_base)};
            if (!dependency) return nullptr;
            if (missing_seal != nullptr) {
                *missing_seal = *dependency;
            }
            if (status != nullptr) {
                *status = PaymentAuditRosterBuildStatus::MISSING_SEAL;
            }
            return nullptr;
        }
        seal_context = accepted_seal_record->verification_context;
    }
    const CBlockIndex* response{
    ...
    const LocalPaymentAuditFinalization* local_finalization)
{
    ...
    if (historical) {
        const auto replay{TryProcessPaymentAuditHistoricalReplay(audit, *historical)};
        if (replay.has_value()) {
            ...
            if (*replay) {
                ...
                LogPrint(BCLog::CHAINLOCKS,
                         "CChainLocksHandler::%s -- accepted historical payment-audit replay proof %s\n",
                         __func__, witness_id.ToString());
            } else {
                fail_request();
            }
    ...
    if (!m_payment_audit_store || !m_payment_audit_store->IsHealthy()) {
        fail_request();
        return;
    }
    ...
    if (!rosters) {
        if (roster_status ==
            PaymentAuditRosterBuildStatus::MISSING_SEAL) {
            if (historical && missing_seal &&
                StagePendingPaymentAuditSealDependency(*missing_seal)) {
                // The peer supplied the exact requested audit. Its locally
                // missing seal is a separate dependency, not a source
                // failure; leave the receipt pending and permit an immediate
                // retry once that seal is admitted.
                complete_request(witness_id);
                return;
            }
            fail_request();
    ...
    })};
    {
        ...
        if (m_pending_payment_audit_receipt &&
            m_pending_payment_audit_receipt
                    ->receipt.audit_witness_id == witness_id &&
            (!historical ||
             *m_pending_payment_audit_receipt == historical->dependency)) {
            m_pending_payment_audit_receipt.reset();
            m_pending_payment_audit_last_request =
                std::chrono::microseconds{0};
            m_retry_pending_btcc_block.store(true);
    ...
    bool* peer_fault)
{
    ...
    if (!accepted) {
        LogPrint(BCLog::CHAINLOCKS, "CChainLocksHandler::%s -- CLSIG %s at %d deferred/rejected: %s\n",
                 __func__, chainlock.GetLogicalId(m_genesis_hash).ToString(),
                 chainlock.statement.height, state.ToString());
    }
    return accepted;
}
    ...
    bool* peer_fault)
{
    ...
    const uint256 witness_id{chainlock.GetWitnessId(m_genesis_hash)};
    const auto capability{
        GetPaymentAuditSealFetchCapability(logical_id)};
    if (!capability) return std::nullopt;
    const pq::RosterAuthorizationBaseIdentity received{
        chainlock.statement.height, chainlock.statement.block_hash, logical_id};
    if (received != capability->dependency.RequestedIdentity() ||
        (!capability->dependency.requested_base &&
         chainlock.statement != capability->dependency.statement) ||
        witness_id.IsNull()) {
        if (peer_fault != nullptr) *peer_fault = true;
        FailPeerResponse(from, logical_id);
        return state.Invalid(BlockValidationResult::BLOCK_CHAINLOCK,
                             "pq-clsig-invalid-payment-audit-seal");
    }
    if (m_store->GetVerifiedRosterAuthorizationBase(received)) {
        CompletePaymentAuditSealFetch(*capability);
        CompletePeerResponse(from, logical_id);
        return true;
    }
    ScopedFinalitySnapshotVerificationRetention snapshot_retention{
        deterministicMNManager.get()};
    std::optional<pq::RosterAuthorizationBaseIdentity> missing_base;
    const auto verification{
        BuildPaymentAuditSealVerificationContext(
            *capability, chainlock.statement, /*publish_roster=*/false,
            &missing_base)};
    if (!verification) {
        const bool advanced{missing_base &&
            AdvancePaymentAuditSealDependency(*capability, *missing_base)};
        CompletePeerResponse(from, logical_id);
        if (advanced) {
            // Retain only the exact older identity. The current multi-megabyte
            // witness must be requested again after its ordinary base verifies.
            return true;
        }
        return state.Error("pq-clsig-payment-audit-seal-context-unavailable");
    }
    pq::ChainLockVerificationError verification_error{
        pq::ChainLockVerificationError::NONE};
    auto checks{pq::PrepareFinalChainLockVerification(
        chainlock, *verification->prepared_context,
        &verification_error)};
    if (!checks) {
        ...
        FailPeerResponse(from, logical_id);
        return state.Invalid(BlockValidationResult::BLOCK_CHAINLOCK,
                             "pq-clsig-invalid-payment-audit-seal-context");
    }
    if (peer_fault != nullptr) *peer_fault = true;
    bool signatures_valid{false};
    {
        LOCK(m_verification_mutex);
        signatures_valid =
            m_verifier.VerifyChecks(std::move(checks->checks));
    }
    if (!signatures_valid) {
        m_store->RejectWitness(chainlock);
        FailPeerResponse(from, logical_id);
        return state.Invalid(BlockValidationResult::BLOCK_CHAINLOCK,
                             "pq-clsig-invalid-payment-audit-seal-signatures");
    }
    ...
    const bool accepted{
        m_chainman.ActiveChainstate().RunWithStableActiveChain([&]() EXCLUSIVE_LOCKS_REQUIRED(!cs_main) {
            if (!IsPaymentAuditSealFetchCapabilityCurrent(*capability)) {
                finality_error =
            ...
            const auto current{
                BuildPaymentAuditSealVerificationContext(
                    *capability, chainlock.statement, /*publish_roster=*/true)};
            if (!current ||
                current->roster_source_generation !=
                    verification->roster_source_generation ||
                !IsQuorumRosterSourceGenerationCurrent(
                    current->roster_source_generation) ||
                current->prepared_context->AuthorizationMask() !=
                    verification->prepared_context->AuthorizationMask() ||
                Descriptors(current->prepared_context->Rosters()) !=
                    Descriptors(
                        verification->prepared_context->Rosters())) {
                finality_error =
                    pq::ChainLockFinalityError::CONTEXT_CHANGED;
                return false;
            }
            ...
            const auto authorize_durable = [&] (
                ...
                return AuthorizePaymentAuditSealPersistence(
                    *capability, persist_record, error);
            };
    ...
    if (index_persistence_failed) {
        ...
        DisableShareAdmission();
        return state.Error(
            "pq-clsig-payment-audit-seal-index-persistence-failed");
    }
    if (!accepted) {
        CompletePeerResponse(from, logical_id);
        return state.Invalid(
            BlockValidationResult::BLOCK_CHAINLOCK,
            strprintf("pq-clsig-payment-audit-seal-%s",
                      FinalityErrorString(finality_error)));
    }

    CompletePaymentAuditSealFetch(*capability);
    m_peerman.RelayInv(CInv{MSG_CLSIG, logical_id});
    CompletePeerResponse(from, logical_id);
    ForgetAllRequests(logical_id);
    LogPrint(BCLog::CHAINLOCKS,
             "CChainLocksHandler::%s accepted payment-audit dependency CLSIG "
             "%s at height %d as authorization only\n",
    ...
    bool retained_local_promotion)
{
    ...
    // The exact payment dependency owns this response even if these bytes
    // also name historical B. Only fresh ordinary verification may persist
    // its authorization edge; the PoW-history import below cannot supply it.
    if (!retained_local_promotion) {
    ...
    if (!retained_local_promotion && !local_finalization && !continuation) {
        {
            LOCK(cs_main);
            const CBlockIndex* tip{m_chainman.ActiveTip()};
            const auto latest{tip ? pq::LatestEligibleChainLockTargetHeight(
                m_config->chainlock_schedule, tip->nHeight) : std::nullopt};
            const auto imported{GetPoWHistoricalSyncAuthorization()};
            if (m_catchup_retry_requested && m_catchup_candidate_hint &&
                !IsRequiredChainLockCatchupCertificate(m_catchup_candidate_hint->logical_id)) {
                m_catchup_candidate_hint.reset();
                m_catchup_retry_requested = false;
            }
            if (!m_catchup_retry_requested && latest &&
                chainlock.IsStructurallyValid() && chainlock.statement.height == *latest &&
                (!imported || chainlock.statement.roster_authorization_base !=
                    imported->base.metadata.AuthorizationBase()) &&
                tip->GetAncestor(*latest)->GetBlockHash() == chainlock.statement.block_hash) {
                m_catchup_candidate_hint = pq::RosterAuthorizationBaseIdentity{
                    *latest, chainlock.statement.block_hash, logical_id};
            }
        }
        if (const auto imported{ProcessPoWHistoricalSyncCertificate(
                from, chainlock, state, peer_fault)}) {
            return *imported;
        }
    }
    ...
    std::optional<pq::VerifiedHistoricalSyncSuccessor> accepted_historical_successor;
    std::shared_ptr<const HistoricalSyncAuthorization> accepted_historical_authorization;
        ...
        if (historical_admission) {
            ...
            accepted = m_chainman.ActiveChainstate().RunWithStableActiveChain(
                [&] {
                    ...
                    std::optional<pq::VerifiedHistoricalSyncSuccessor> historical_successor;
                    std::shared_ptr<const HistoricalSyncAuthorization> historical_authorization;
                    if (catchup) {
                        LOCK(cs_main);
                        if (!PrepareHistoricalSyncSuccessor(chainlock, historical_successor)) {
                            finality_error = pq::ChainLockFinalityError::CONTEXT_CHANGED;
                            m_store->AbandonPrepared(*prepared);
                            return false;
                        }
                        historical_authorization = m_historical_sync;
                    }
                    ...
                    if (catchup) {
                        const auto coverage_authorization{
                            GetReceiptArchiveCoverageAuthorization(
                                *prepared)};
                        const bool saved{m_store->AcceptCatchupVerified(
                            *prepared, chainlock, /*signatures_valid=*/true,
                            ...
                            verification_context->prepared_context,
                            recovery_universe,
                            historical_successor ? &*historical_successor : nullptr)};
                        if (saved && historical_successor) {
                            accepted_historical_successor = historical_successor;
                            accepted_historical_authorization = historical_authorization;
                            LOCK(cs_main);
                            CompleteHistoricalSyncSuccessor(*historical_successor, historical_authorization);
                        }
                        return saved;
                    }
    ...
    const auto accept_verified = [&] {
        ...
        const bool saved{coverage_authorization
    ...
    ForgetAllRequests(logical_id);
    EnforceBestChainLock();
    if (accepted_historical_successor) {
        // A permitted competing C is not active until enforcement. Rebind its
        // same verified prefix before publishing post-enforcement readiness.
        (void)m_chainman.ActiveChainstate().RunWithStableActiveChain([&] {
            LOCK(cs_main);
            CompleteHistoricalSyncSuccessor(*accepted_historical_successor,
                                            accepted_historical_authorization);
            return true;
        });
    }
    MaintainPaymentAuditCheckpointGC();
...
CChainLocksHandler::TryImportPersistedUnsealedBTCC()
{
    ...
    const auto imported_unsealed{m_store->GetUnsealedBTCC()};
    if (imported_unsealed && *imported_unsealed == persisted) {
        const auto publication{
            BeginChainLockAuxiliarySnapshotPublication()};
        if (!publication) {
            return PersistedChainLockImport::PENDING;
...
{
    RevokeReorgedHistoricalSyncAuthorization();
    CheckActiveState();
    (void)RevalidatePendingBTCCReceiptDependency();
    (void)RevalidatePendingPaymentAuditReceiptDependency();
}
...
void CChainLocksHandler::CheckActiveState()
{
    ...
    {
        // Serialize this observation with revocation; a stale health evaluation
        // must not reopen GC after the pending transition closed its gate.
        LOCK(cs_main);
