...
inline constexpr std::size_t QUORUM_MAX_BYZANTINE{
    (QUORUM_SIZE - 1) / 3};
// With n=400 and f=133, any two 267-member certificates over this same roster
// intersect in at least 134 members. A conflicting pair therefore requires an
// honest signer to equivocate when this roster contains at most 133 Byzantine
// members; this does not assert the same intersection across different views.
...
struct RosterBeaconSeed {
     ...
     * RECOVERY: copied from the last durable normal delayed-beacon source.
     */
    int32_t anchor_btc_height{-1};
    /**
     * NORMAL READY: delayed active-Bitcoin F hash.
     * RECOVERY READY: copied delayed F hash; recovery never invents entropy.
     */
    uint256 future_btc_hash;
    uint256 recovery_entropy_hash;
    ...
    [[nodiscard]] bool IsRecovery() const noexcept;
...
struct RecoveryRefreshSource {
    static constexpr std::size_t WIRE_SIZE{4 * sizeof(uint32_t) + 6 * 32};

    uint32_t group{0};
    int32_t snapshot_height{-1};
    uint256 snapshot_hash;
    uint256 universe_root;
    int32_t entropy_height{-1};
    uint256 entropy_block_hash;
    int32_t carrier_height{-1};
    uint256 carrier_block_hash;
    uint256 parent_work_hash;
    uint256 seed;

    SERIALIZE_METHODS(RecoveryRefreshSource, obj)
    {
        READWRITE(obj.group, obj.snapshot_height, obj.snapshot_hash,
                  obj.universe_root, obj.entropy_height,
                  obj.entropy_block_hash, obj.carrier_height,
                  obj.carrier_block_hash, obj.parent_work_hash, obj.seed);
    }

    [[nodiscard]] bool IsNull() const noexcept;
    [[nodiscard]] bool IsStructurallyValid() const noexcept;
    friend bool operator==(const RecoveryRefreshSource&,
                           const RecoveryRefreshSource&) = default;
};

/** The signed provenance is explicit: a refreshed source makes no BTC claim. */
struct RecoveryRosterAuthoritySource {
    static constexpr std::size_t WIRE_SIZE{
        sizeof(uint8_t) + RosterBeaconSeed::WIRE_SIZE +
        RecoveryRefreshSource::WIRE_SIZE};

    RosterBeaconSeed normal_beacon;
    RecoveryRosterSourceKind kind{RecoveryRosterSourceKind::NORMAL_BEACON};
    RecoveryRefreshSource refresh{};

    SERIALIZE_METHODS(RecoveryRosterAuthoritySource, obj)
    {
        uint8_t kind{static_cast<uint8_t>(obj.kind)};
        READWRITE(kind, obj.normal_beacon, obj.refresh);
        SER_READ(obj, obj.kind = static_cast<RecoveryRosterSourceKind>(kind));
        SER_READ(obj, if (!obj.IsStructurallyValid()) {
...
struct ActiveRosterBeaconBundle {
    ...
    /** Exact immutable source fixing the recovery identity universe and entropy. */
    ...
    SERIALIZE_METHODS(ActiveRosterBeaconBundle, obj)
    {
        ...
        READWRITE(obj.recovery_authority_source);
        SER_READ(obj, if (!obj.IsStructurallyValid()) {
            throw std::ios_base::failure(
...
struct BTCCReceiptState {
    static constexpr std::size_t WIRE_SIZE{3 * sizeof(int32_t) + 3 * 32};

    BTCCursor cursor;
    uint256 cumulative_hash;
    /** Target and carrier of the newest non-null receipt in this prefix. */
    int32_t latest_chainlock_target_height{-1};
    int32_t latest_receipt_carrier_height{-1};

    SERIALIZE_METHODS(BTCCReceiptState, obj)
    {
        READWRITE(obj.cursor, obj.cumulative_hash,
                  obj.latest_chainlock_target_height,
                  obj.latest_receipt_carrier_height);
    }
...
static_assert(BTCCReceiptState::WIRE_SIZE == 108);
...
/** Exact certificate whose authenticated roster state authorizes a transition. */
struct RosterAuthorizationBaseIdentity {
    static constexpr std::size_t WIRE_SIZE{sizeof(int32_t) + 2 * 32};
    ...
    SERIALIZE_METHODS(RosterAuthorizationBaseIdentity, obj)
    {
        READWRITE(obj.height, obj.block_hash, obj.logical_id);
    }

    [[nodiscard]] bool IsNull() const noexcept;
    [[nodiscard]] bool IsStructurallyValid() const noexcept;
    friend bool operator==(const RosterAuthorizationBaseIdentity&,
                           const RosterAuthorizationBaseIdentity&) = default;
};

static_assert(RosterAuthorizationBaseIdentity::WIRE_SIZE == 68);
...
struct ChainLockStatement {
    static constexpr std::size_t WIRE_SIZE{
        ...
        RosterAuthorizationBaseIdentity::WIRE_SIZE +
