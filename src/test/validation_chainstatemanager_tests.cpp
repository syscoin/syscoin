...
struct MiningNEVMPrefixSetup : CommittedNEVMContinuitySetup {
    ...
    {
        BOOST_CHECK(block.hashPrevBlock == original_tip->GetBlockHash());
        const dev::RLP encoded{block.vchNEVMBlockData};
...
struct LostAckMiningNEVMPrefixSetup : MiningNEVMPrefixSetup {
    ...
    enum class OwnershipFault { FOREIGN_SIBLING, FAILED, REMOVED, MISSING_DATA, CONFLICT, SELECTED_SIBLING };
    ...
    void CheckWorkerRejectsLostAckOwnership(OwnershipFault fault)
    {
        ...
        if (fault == OwnershipFault::FOREIGN_SIBLING || fault == OwnershipFault::SELECTED_SIBLING) {
            const auto* sibling{StoreSibling(fault == OwnershipFault::SELECTED_SIBLING)};
...
struct NEVMPayloadForkSetup : NEVMPayloadRepairSetup {
                           ...
                           bool applied_suffix = false)
    {
        ...
        BOOST_CHECK(state.IsValid());
        BOOST_CHECK_EQUAL(m_node.exit_status.load(), EXIT_SUCCESS);
        // An above-P fork retains an unapplied common ancestor. Its first
        // connect recovers that shared prefix after the local suffix undo.
        BOOST_CHECK_EQUAL(nevm->block_info_queries, queries + (above_applied ? 3U : 1U));
        BOOST_CHECK_EQUAL(nevm->flush_requests, flushes + (above_applied ? 3U : 1U));
                        ...
                        NEVMOperationalForkSetup)
{
    ...
    BOOST_CHECK(nevm->last_reported_pair->hash == prefix[2]->GetHash());
}
...
BOOST_FIXTURE_TEST_CASE(nevm_mining_worker_lost_ack_rejects_changed_selection, LostAckMiningNEVMPrefixSetup)
{
    CheckWorkerRejectsLostAckOwnership(OwnershipFault::SELECTED_SIBLING);
}
