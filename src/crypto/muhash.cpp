...
// SYSCOIN BEGIN: Backport Bitcoin Core a26ce628942243fc9848a63bfdfa5e61f5e936f3
// (safegcd modular inversion). Preserve the existing MuHash digest and wire
// representation while avoiding thousands of squarings per state commitment.
namespace {
...
{
    // SYSCOIN: Preserve the previous Fermat implementation's zero result for
    // division by zero (including a denominator equal to the modulus).
    limb_t nonzero{0};
    for (const limb_t limb : limbs) nonzero |= limb;
    if (nonzero == 0) return *this;
...
// SYSCOIN END: Bitcoin Core safegcd inversion backport.
