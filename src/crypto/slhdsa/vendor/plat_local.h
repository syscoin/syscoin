 ...
 *
 * Modified by the Syscoin project: detect byte order explicitly for portable
 * address and SHAKE lane encodings, and fail closed when it is unknown.
 */
...
/* SYSCOIN: Both supported byte orders retain canonical FIPS byte encodings. */
...
#elif defined(_WIN32)
/* All Windows targets supported by Syscoin are little-endian. */
#define PLAT_LITTLE_ENDIAN
#else
#error "slhdsa-c: unsupported platform byte-order detection"
#endif
