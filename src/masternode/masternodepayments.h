...
/** Current next-block superblock policy and outputs; nullopt means unavailable.
 * The caller must hold cs_main. Ordinary heights and disabled policy return zero.
 */
std::optional<uint256> GetMiningPaymentFingerprint(const CBlockIndex& parent) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
bool FillBlockPayments(CChain& activeChain, CMutableTransaction& txNew, int nBlockHeight, const CAmount &blockReward, const CAmount &fees, std::vector<CTxOut>& voutMasternodePaymentsRet, std::vector<CTxOut>& voutSuperblockPaymentsRet, uint256* superblock_payment_hash = nullptr);
