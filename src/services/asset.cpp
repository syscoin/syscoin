...
void CAuxFeeDetails::ToJson(UniValue& value, const uint32_t& nBaseAsset) const {
    ...
    for(const auto& auxfee: vecAuxFees) {
        ...
        auxfeeObj.pushKV("bound", ValueFromAmount(auxfee.nBound, nBaseAsset));
        auxfeeObj.pushKV("percentage", strprintf("%.5f", auxfee.nPercent / 100000.0));
        feeStruct.push_back(auxfeeObj);
    }
    value.pushKV("auxfee_address", vchAuxFeeKeyID.empty()? "" : EncodeDestination(WitnessV0KeyHash(uint160{vchAuxFeeKeyID})));
