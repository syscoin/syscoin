...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            if redirect_stderr:
                stderr = sys.stdout
            syscoind_processes[i] = subprocess.Popen(args, stderr=stderr)
...
def stop_node(node, i):
    ...
    return_code = syscoind_processes[i].wait(timeout=syscoinD_PROC_WAIT_TIMEOUT)
...
def stop_nodes(nodes):
    for i, node in enumerate(nodes):
        stop_node(node, i)
    assert not syscoind_processes.values() # All connections must be gone now
